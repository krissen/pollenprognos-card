...
class PollenPrognosCardEditor extends LitElement {
  ...
  _updateConfig(prop, value) {
    ...
    // Handle color mode changes - reset to defaults when switching from custom to default
    if (prop === "allergen_color_mode" && value === "default_colors" && this._config.allergen_color_mode === "custom") {
      // Reset allergen colors to defaults
      const newConfig = {
        ...this._config,
    ...
    if (prop === "levels_inherit_mode" && value === "inherit_allergen" && this._config.levels_inherit_mode === "custom") {
      // Reset level colors to defaults when switching to inherit mode
      const newConfig = {
        ...this._config,
        ...
        <details open>
          ...
          <details open>
            ...
            <details>
              ...
              <!-- Allergen Stroke Width (always visible, not synced with level circles) -->
              <ha-formfield
                label="${this._t("allergen_stroke_width") || "Stroke Width"}"
              >
                <div class="slider-row">
                  <div class="slider-text">${this._t("allergen_stroke_width") || "Stroke Width"}</div>
                  <div class="slider-value">${c.allergen_stroke_width || LEVELS_DEFAULTS.allergen_stroke_width}</div>
                  <ha-slider
                    min="0"
                    max="100"
                    step="1"
              ...
              <!-- Synced with Allergen Colors Summary -->
              <details>
                <summary>${this._t("allergen_sync_summary") || "Syncs with Allergen"}</summary>
            ...
            <details>
              ...
              <!-- Synced with Allergen Summary -->
              <details>
                <summary>${this._t("levels_sync_summary") || "Syncs with Allergen"}</summary>
                ...
                <div style="${c.levels_inherit_mode === "custom" ? "" : "display: none;"}">
                  ...
                  >
                    <div class="slider-row">
                      <div class="slider-text">${this._t("levels_thickness")}</div>
                      <div class="slider-value">${c.levels_thickness}</div>
                      <ha-slider
                        min="10"
                        max="90"
                        step="1"
              ...
              <ha-formfield label="${this._t("levels_gap")}">
                <div class="slider-row">
                  <div class="slider-text">${this._t("levels_gap")}</div>
                  <div class="slider-value">${c.levels_gap}</div>
                  <ha-slider
                    min="0"
                    max="20"
                    step="1"
              ...
              <ha-formfield label="${this._t("levels_text_size")}">
                <div class="slider-row">
                  <div class="slider-text">${this._t("levels_text_size")}</div>
                  <div class="slider-value">${(c.levels_text_size || 0.3).toFixed(2)}</div>
                  <ha-slider
                    min="0.1"
                    max="0.5"
                    step="0.05"
              ...
              <ha-formfield label="${this._t("levels_icon_ratio")}">
                <div class="slider-row">
                  <div class="slider-text">${this._t("levels_icon_ratio")}</div>
                  <div class="slider-value">${(c.levels_icon_ratio || 1).toFixed(2)}</div>
                  <ha-slider
                    min="0.1"
                    max="2"
                    step="0.05"
