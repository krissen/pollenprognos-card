...
class PollenPrognosCardEditor extends LitElement {
  ...
  set hass(hass) {
    ...
    if (!explicit) {
      ...
      this._userConfig.integration = integration;
      if (this.debug)
        console.debug("[Editor] autodetect:", { pp: ppStates.length, plu: pluStates.length, peu: peuStates.length, dwd: dwdStates.length, silam: silamStates.length, atmo: atmoDiscovery.locations.size, gpl: gplStates.length, chosen: integration });
    }
