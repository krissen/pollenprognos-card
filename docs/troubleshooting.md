...
- One exception to free renaming: the per-allergen **detail sensors** (disabled by default) are all registered under the same translation key, so the allergen is read from the entity ID or the friendly name. Home Assistant's frontend does not always expose the registry `unique_id` that would identify them otherwise, so keep the allergen word in one of those two names if you enable them. Per-allergen data coming from the category sensors is unaffected
