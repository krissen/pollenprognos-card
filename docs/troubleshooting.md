...
4. **Sensors renamed in Home Assistant.** Since v3.2.0 the card uses device-registry-based discovery as its primary path for PP, DWD, PEU, SILAM, Atmo, GPL, GP and MSW, and Kleenex now uses it too, so renamed entity IDs no longer break detection. Kleenex entities are identified by their registry entry rather than by the older `sensor.kleenex_pollen_radar_<location>_<category>` pattern, and locations are matched on the device (its identifier, then its name), so renaming either the device or its entities keeps working. If discovery fails on any adapter for some other reason, two fallbacks remain:
...
6. **Integration reinstalled with a new `config_entry_id`.** If your card was configured (via the visual editor) with a `config_entry_id` and you later removed and re-added the integration, the saved ID will no longer match any discovered location. The card auto-recovers for all adapters that accept a `config_entry_id` (DWD, GPL, GP, SILAM, Atmo and MSW via an explicit retry-as-autodetect path, and PP and PEU via their legacy template-fallback path) by falling back to the first discovered location. No user action is required. To make the new `entry_id` permanent in your config, open the card in the visual editor and re-pick the location. (Kleenex normally stores the device's own location name instead of a `config_entry_id`, and that name is recreated unchanged when the integration is re-added, so saved Kleenex configs are unaffected. A Kleenex device that exposes no usable identifier falls back to a `config_entry_id`; such a config is then matched on the device name instead.)
...
- Pick the location in the visual editor's dropdown, or set `location` to the name you gave the integration instance (e.g. `utrecht`), the device name, or its `config_entry_id`. If two instances share a name, the card renders nothing for that value rather than guessing between them: rename one of the devices to tell them apart
- Multi-location setups are supported via separate config entries
- `location: manual` with `entity_prefix`/`entity_suffix` remains available as an override when discovery cannot see the entities at all
