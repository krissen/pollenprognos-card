...
- If two instances share a name, the card renders nothing for that value rather than guessing between them: rename one of the devices to tell them apart
...
- **US zones**: the upstream API does not return per-allergen breakdowns for North America, only the category totals for trees, grasses and weeds. If your config lists individual allergens (e.g. `birch`, `oak`, `chenopod`), none of them can be filled, so from card v4.0.1 the card shows those three category totals instead and says so once in the console. You do not have to change anything; setting `allergens: [trees_cat, grass_cat, weeds_cat]` (they are in the editor's allergen picker) makes the same result explicit and silences the message. A category reading 0 ppm across the whole forecast is hidden by `pollen_threshold`, exactly like any allergen that stays at zero; set `pollen_threshold: 0` to show it anyway. Configuring a raw category name (`trees` rather than `trees_cat`) keeps the card on that key alone and leaves the other categories out.
