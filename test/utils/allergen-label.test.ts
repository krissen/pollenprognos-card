...
describe("resolveAllergenPhrase", () => {
  it("returns the editor.phrases_full string for a covered key", () => {
    expect(resolveAllergenPhrase("birch", "birch", { lang: "en" })).toBe(
      "Birch",
    );
    expect(resolveAllergenPhrase("birch", "birch", { lang: "sv" })).toBe(
      "Björk",
    );
  });
  ...
  it("uses the phrases_short namespace when short:true", () => {
    ...
    expect(
      resolveAllergenPhrase("grass", "grass", { short: true, lang: "en" }),
    ).toBe("Grass");
  });

  it("resolves graminales (regression: used to leak the raw key)", () => {
    expect(
      resolveAllergenPhrase("graminales", "graminales", { lang: "es" }),
    ).toBe("Gramíneas");
    expect(
      resolveAllergenPhrase("graminales", "graminales", { lang: "de" }),
    ).toBe("Gräser");
    expect(
      resolveAllergenPhrase("graminales", "graminales", { lang: "en" }),
    ).toBe("Grasses");
  });
  ...
  it("falls back to card.allergen when the editor phrase is missing", () => {
    ...
    const out = resolveAllergenPhrase("allergy_risk", "allergy_risk", {
      lang: "en",
    });
  ...
  it("never returns a raw i18n key for an unknown allergen", () => {
    const out = resolveAllergenPhrase(
  ...
  it("humanizes from raw, capitalizing and replacing underscores", () => {
    expect(resolveAllergenPhrase("xx_yy", "xx_yy", { lang: "en" })).toBe(
      "Xx yy",
    );
  });
  ...
  it("handles null/empty canonical and raw without throwing", () => {
    ...
    expect(
      resolveAllergenPhrase(undefined as any, undefined as any, { lang: "en" }),
    ).toBe("");
    expect(resolveAllergenPhrase("foo", null as any, { lang: "en" })).toBe(
      "Foo",
    );
  });
  ...
  it("falls back to English locale data for an unknown language", () => {
    expect(
      resolveAllergenPhrase("graminales", "graminales", { lang: "xx" }),
    ).toBe("Grasses");
  });
});
