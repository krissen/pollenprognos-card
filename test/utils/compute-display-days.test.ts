...
function makeSensor(validDays: number, invalidDays = 0): any {
  const days = [];
...
describe("computeDisplayDays: IRM KMI integration", () => {
  it("clamps to 1 regardless of days_to_show", () => {
    const cfg: any = { integration: "irmkmi", days_to_show: 5 };
    expect(computeDisplayDays([sensor5], cfg)).toBe(1);
  });
...
describe("computeDisplayDays: sensors without days array", () => {
  it("returns 0 for empty sensor list", () => {
    expect(computeDisplayDays([], { days_to_show: 5 } as any)).toBe(0);
  });

  it("skips sensors that have no days property", () => {
    const noDays: any = { allergenReplaced: "birch" }; // no .days
    expect(computeDisplayDays([noDays], { days_to_show: 5 } as any)).toBe(0);
  });

  it("skips sensors with empty days array", () => {
    const emptyDays: any = { days: [] };
    expect(computeDisplayDays([emptyDays], { days_to_show: 5 } as any)).toBe(0);
  });
