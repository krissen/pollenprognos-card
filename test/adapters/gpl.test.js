...
describe("fetchForecast: forecast data", () => {
  ...
  // Issue #271: pollenlevels' `offset` is relative to the integration's last
  // fetch day and can lag the user's local day, so an item with offset=1 may
  // carry today's date. The date is the ground truth; a stale item must not
  // produce a second "Today" column.
  it("does not duplicate today when a stale offset=1 item carries today's date (#271)", async () => {
  ...
  it("places a forecast item by its date even when the offset is stale", async () => {
  ...
  it("backfills day0 from a today-dated forecast item when the state is unavailable", async () => {
