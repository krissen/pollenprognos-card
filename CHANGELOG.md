...
- **Per-scale level-name i18n keys.** New `card.levels5.0..4` and `editor.phrases_levels5.0..4` keys in all 15 locale files cover the five-level severity scale, sourced from each language's existing `card.levels.{0,1,3,5,6}` strings (the same translations users have seen via PEU's runtime spread for years). MSW reads these via a new `buildLevelNamesForScale(scale, ...)` helper. PEU's runtime spread is kept for now; migration tracked in #215.
