 ...
 */
var xA;
const pA = window, aA = pA.trustedTypes, VA = aA ? aA.createPolicy("lit-html", { createHTML: (t) => t }) : void 0, HA = "$lit$", _ = `lit$${(Math.random() + "").slice(9)}$`, rg = "?" + _, ug = `<${rg}>`, IA = document, DA = () => IA.createComment(""), dA = (t) => t === null || typeof t != "object" && typeof t != "function", og = Array.isArray, fg = (t) => og(t) || typeof (t == null ? void 0 : t[Symbol.iterator]) == "function", wA = `[ 	
\f\r]`, sA = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, zA = /-->/g, NA = />/g, gA = RegExp(`>|${wA}(?:([^\\s"'>=/]+)(${wA}*=${wA}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), XA = /'/g, qA = /"/g, lg = /^(?:script|style|textarea|title)$/i, mg = (t) => (A, ...g) => ({ _$litType$: t, strings: A, values: g }), y = mg(1), rA = Symbol.for("lit-noChange"), F = Symbol.for("lit-nothing"), _A = /* @__PURE__ */ new WeakMap(), eA = IA.createTreeWalker(IA, 129, null, !1);
function sg(t, A) {
        ...
        if (n.hasAttributes()) {
          ...
          for (const r of n.getAttributeNames()) if (r.endsWith(HA) || r.startsWith(_)) {
            ...
            if (h.push(r), l !== void 0) {
              ...
              i.push({ type: 1, index: C, name: x[2], strings: E, ctor: x[1] === "." ? xg : x[1] === "?" ? yg : x[1] === "@" ? vg : fA });
...
const wg = aA ? aA.emptyScript : "";
class yg extends fA {
  constructor() {
    super(...arguments), this.type = 4;
  }
  j(A) {
    A && A !== F ? this.element.setAttribute(this.name, wg) : this.element.removeAttribute(this.name);
  }
}
class vg extends fA {
  constructor(A, g, e, n, C) {
...
iA.finalized = !0, iA._$litElement$ = !0, (yA = globalThis.litElementHydrateSupport) === null || yA === void 0 || yA.call(globalThis, { LitElement: iA });
...
const bA = (t, A = "_") => {
  ...
  __proto__: null,
  default: Fn
}, Symbol.toStringTag, { value: "Module" })), eg = /* @__PURE__ */ Object.assign({ "./locales/cs.json": Bn, "./locales/da.json": xn, "./locales/de.json": yn, "./locales/en.json": Rn, "./locales/fi.json": kn, "./locales/it.json": bn, "./locales/nl.json": Jn, "./locales/no.json": Yn, "./locales/ru.json": Gn, "./locales/sk.json": Wn, "./locales/sv.json": Ln }), lA = {};
for (const t in eg) {
...
async function qn(t, A) {
  ...
  for (const c of A.allergens)
    try {
      ...
      const m = t.states[S], w = j(m.state), v = j(m.attributes[Tn]), f = j(m.attributes[Vn]), J = [
        { date: Y, level: w },
  ...
  for (const d of A.allergens)
    try {
      ...
      if (A.allergens_abbreviated) {
        ...
        S.allergenShort = S.allergenCapitalized;
      let w = c ? `sensor.polleninformation_${c}_${m}` : null;
      if (!w || !t.states[w]) {
        const Q = M.filter((R) => {
...
async function nt(t, A) {
  ...
  for (const s of A.allergens)
    try {
      ...
      const m = t.states[S], w = Number(m.state);
      let v = [];
      ...
      let f = [];
      if (f.push(CA(s, w)), m.attributes && Array.isArray(m.attributes.forecast) && m.attributes.forecast.length > 0)
        for (let k = 1; k < l; ++k) {
  ...
  for (const B of l)
    try {
      ...
      for (let s = 0; s < o.length; ++s) {
        const p = o[s], u = `pollen_${B}`, d = Number(p[u]), S = CA(B, d), m = S < 0 ? r : h[S] || String(S), w = new Date(p.datetime || p.time);
        let v, f;
        if (A.mode === "twice_daily") {
          const J = w.toLocaleDateString(C, { weekday: "short" });
...
async function at(t, A) {
  ...
  const g = [], e = !!A.debug, n = (u) => u.charAt(0).toUpperCase() + u.slice(1), C = (u) => {
    const [d] = u.split("T"), [S, m, w] = d.split("-").map(Number);
    return new Date(S, m - 1, w);
  }, a = /* @__PURE__ */ new Date();
  a.setHours(0, 0, 0, 0);
...
class rt extends iA {
  ...
  set hass(g) {
    ...
    if (this.config = r, this.tapAction = r.tap_action || this.tapAction || null, this.debug && (console.debug("[Card][Debug] Aktiv integration:", i), console.debug("[Card][Debug] Allergens i config:", r.allergens)), r.title === "false" || r.title === !1 || typeof r.title == "string" && r.title.trim() === "")
      ...
      if (i === "dwd")
        ...
        ), u = Object.values(g.states).filter((w) => {
          if (!w.entity_id.startsWith("sensor.silam_pollen_")) return !1;
          const v = w.entity_id.match(
            /^sensor\.silam_pollen_(.*)_([^_]+)$/
          );
          if (!v) return !1;
          const f = v[2];
          return p.has(f);
        }), d = bA(r.location || ""), S = u.find((w) => {
          const f = w.entity_id.replace("sensor.silam_pollen_", "").replace(/_[^_]+$/, "").replace(/^[-\s]+/, "");
          return bA(f) === d;
        });
        let m = "";
        if (S) {
          const w = S.attributes;
          m = w.location_title || ((M = w.friendly_name) == null ? void 0 : M.replace(/^SILAM Pollen\s*-?\s*/i, "").replace(new RegExp("\\s+\\p{L}+$", "u"), "").trim()) || r.location, m = m.replace(/^[-\s]+/, "");
        }
        c = m || r.location || "";
      } else
        c = JA.find(
    ...
    if (E)
      return E.then((c) => {
        ...
        if (r.integration === "silam") {
          const w = Object.keys(g.states).filter((v) => {
        ...
        let d = c.filter((w) => {
          if (r.integration === "silam" && u && (!r.mode || r.mode === "daily")) {
            const v = r.location || "", f = u[w.allergenReplaced] || w.allergenReplaced, J = `sensor.silam_pollen_${v}_${f}`;
            return this.debug && console.debug(
              `[Card][Debug][SILAM filter] allergenReplaced: '${w.allergenReplaced}', key: '${f}', id: '${J}', available: ${s.includes(J)}`
            ), s.includes(J);
          }
          return !0;
        });
        if (Array.isArray(r.allergens) && r.allergens.length > 0 && r.integration !== "silam") {
          let w, v;
          i === "dwd" ? (w = new Set(r.allergens.map((f) => uA(f))), v = (f) => uA(f.allergenReplaced || "")) : (this.debug && console.debug(
            "[Card][Debug] Använder normalisering för allergener:",
            r.allergens
          ), w = new Set(r.allergens.map((f) => $(f))), v = (f) => $(f.allergenReplaced || "")), d = d.filter((f) => {
            const J = v(f), k = w.has(J);
  ...
  render() {
    ...
    if (this.sensors.length)
      ...
      let h = "";
      this._availableSensorCount === 0 ? h = this._t("card.error_no_sensors") : h = this._t("card.error_filtered_sensors"), g = y`<div class="card-error">${h} (${D})</div>`;
    }
