var Dg = Object.defineProperty;
var eg = (e, A, g) => A in e ? Dg(e, A, { enumerable: !0, configurable: !0, writable: !0, value: g }) : e[A] = g;
var vA = (e, A, g) => eg(e, typeof A != "symbol" ? A + "" : A, g);
const qA = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  get fetchForecast() {
    return vC;
  },
  get stubConfigPP() {
    return w;
  }
}, Symbol.toStringTag, { value: "Module" }));
/**
 * @license
 * Copyright 2019 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const sA = window, RA = sA.ShadowRoot && (sA.ShadyCSS === void 0 || sA.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, wA = Symbol(), kA = /* @__PURE__ */ new WeakMap();
let zA = class {
  constructor(A, g, I) {
    if (this._$cssResult$ = !0, I !== wA) throw Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");
    this.cssText = A, this.t = g;
  }
  get styleSheet() {
    let A = this.o;
    const g = this.t;
    if (RA && A === void 0) {
      const I = g !== void 0 && g.length === 1;
      I && (A = kA.get(g)), A === void 0 && ((this.o = A = new CSSStyleSheet()).replaceSync(this.cssText), I && kA.set(g, A));
    }
    return A;
  }
  toString() {
    return this.cssText;
  }
};
const hg = (e) => new zA(typeof e == "string" ? e : e + "", void 0, wA), NA = (e, ...A) => {
  const g = e.length === 1 ? e[0] : A.reduce((I, n, C) => I + ((D) => {
    if (D._$cssResult$ === !0) return D.cssText;
    if (typeof D == "number") return D;
    throw Error("Value passed to 'css' function must be a 'css' function result: " + D + ". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.");
  })(n) + e[C + 1], e[0]);
  return new zA(g, e, wA);
}, tg = (e, A) => {
  RA ? e.adoptedStyleSheets = A.map((g) => g instanceof CSSStyleSheet ? g : g.styleSheet) : A.forEach((g) => {
    const I = document.createElement("style"), n = sA.litNonce;
    n !== void 0 && I.setAttribute("nonce", n), I.textContent = g.cssText, e.appendChild(I);
  });
}, PA = RA ? (e) => e : (e) => e instanceof CSSStyleSheet ? ((A) => {
  let g = "";
  for (const I of A.cssRules) g += I.cssText;
  return hg(g);
})(e) : e;
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
var fA;
const rA = window, OA = rA.trustedTypes, ig = OA ? OA.emptyScript : "", YA = rA.reactiveElementPolyfillSupport, uA = { toAttribute(e, A) {
  switch (A) {
    case Boolean:
      e = e ? ig : null;
      break;
    case Object:
    case Array:
      e = e == null ? e : JSON.stringify(e);
  }
  return e;
}, fromAttribute(e, A) {
  let g = e;
  switch (A) {
    case Boolean:
      g = e !== null;
      break;
    case Number:
      g = e === null ? null : Number(e);
      break;
    case Object:
    case Array:
      try {
        g = JSON.parse(e);
      } catch {
        g = null;
      }
  }
  return g;
} }, _A = (e, A) => A !== e && (A == A || e == e), UA = { attribute: !0, type: String, converter: uA, reflect: !1, hasChanged: _A }, pA = "finalized";
let gA = class extends HTMLElement {
  constructor() {
    super(), this._$Ei = /* @__PURE__ */ new Map(), this.isUpdatePending = !1, this.hasUpdated = !1, this._$El = null, this._$Eu();
  }
  static addInitializer(A) {
    var g;
    this.finalize(), ((g = this.h) !== null && g !== void 0 ? g : this.h = []).push(A);
  }
  static get observedAttributes() {
    this.finalize();
    const A = [];
    return this.elementProperties.forEach((g, I) => {
      const n = this._$Ep(I, g);
      n !== void 0 && (this._$Ev.set(n, I), A.push(n));
    }), A;
  }
  static createProperty(A, g = UA) {
    if (g.state && (g.attribute = !1), this.finalize(), this.elementProperties.set(A, g), !g.noAccessor && !this.prototype.hasOwnProperty(A)) {
      const I = typeof A == "symbol" ? Symbol() : "__" + A, n = this.getPropertyDescriptor(A, I, g);
      n !== void 0 && Object.defineProperty(this.prototype, A, n);
    }
  }
  static getPropertyDescriptor(A, g, I) {
    return { get() {
      return this[g];
    }, set(n) {
      const C = this[A];
      this[g] = n, this.requestUpdate(A, C, I);
    }, configurable: !0, enumerable: !0 };
  }
  static getPropertyOptions(A) {
    return this.elementProperties.get(A) || UA;
  }
  static finalize() {
    if (this.hasOwnProperty(pA)) return !1;
    this[pA] = !0;
    const A = Object.getPrototypeOf(this);
    if (A.finalize(), A.h !== void 0 && (this.h = [...A.h]), this.elementProperties = new Map(A.elementProperties), this._$Ev = /* @__PURE__ */ new Map(), this.hasOwnProperty("properties")) {
      const g = this.properties, I = [...Object.getOwnPropertyNames(g), ...Object.getOwnPropertySymbols(g)];
      for (const n of I) this.createProperty(n, g[n]);
    }
    return this.elementStyles = this.finalizeStyles(this.styles), !0;
  }
  static finalizeStyles(A) {
    const g = [];
    if (Array.isArray(A)) {
      const I = new Set(A.flat(1 / 0).reverse());
      for (const n of I) g.unshift(PA(n));
    } else A !== void 0 && g.push(PA(A));
    return g;
  }
  static _$Ep(A, g) {
    const I = g.attribute;
    return I === !1 ? void 0 : typeof I == "string" ? I : typeof A == "string" ? A.toLowerCase() : void 0;
  }
  _$Eu() {
    var A;
    this._$E_ = new Promise((g) => this.enableUpdating = g), this._$AL = /* @__PURE__ */ new Map(), this._$Eg(), this.requestUpdate(), (A = this.constructor.h) === null || A === void 0 || A.forEach((g) => g(this));
  }
  addController(A) {
    var g, I;
    ((g = this._$ES) !== null && g !== void 0 ? g : this._$ES = []).push(A), this.renderRoot !== void 0 && this.isConnected && ((I = A.hostConnected) === null || I === void 0 || I.call(A));
  }
  removeController(A) {
    var g;
    (g = this._$ES) === null || g === void 0 || g.splice(this._$ES.indexOf(A) >>> 0, 1);
  }
  _$Eg() {
    this.constructor.elementProperties.forEach((A, g) => {
      this.hasOwnProperty(g) && (this._$Ei.set(g, this[g]), delete this[g]);
    });
  }
  createRenderRoot() {
    var A;
    const g = (A = this.shadowRoot) !== null && A !== void 0 ? A : this.attachShadow(this.constructor.shadowRootOptions);
    return tg(g, this.constructor.elementStyles), g;
  }
  connectedCallback() {
    var A;
    this.renderRoot === void 0 && (this.renderRoot = this.createRenderRoot()), this.enableUpdating(!0), (A = this._$ES) === null || A === void 0 || A.forEach((g) => {
      var I;
      return (I = g.hostConnected) === null || I === void 0 ? void 0 : I.call(g);
    });
  }
  enableUpdating(A) {
  }
  disconnectedCallback() {
    var A;
    (A = this._$ES) === null || A === void 0 || A.forEach((g) => {
      var I;
      return (I = g.hostDisconnected) === null || I === void 0 ? void 0 : I.call(g);
    });
  }
  attributeChangedCallback(A, g, I) {
    this._$AK(A, I);
  }
  _$EO(A, g, I = UA) {
    var n;
    const C = this.constructor._$Ep(A, I);
    if (C !== void 0 && I.reflect === !0) {
      const D = (((n = I.converter) === null || n === void 0 ? void 0 : n.toAttribute) !== void 0 ? I.converter : uA).toAttribute(g, I.type);
      this._$El = A, D == null ? this.removeAttribute(C) : this.setAttribute(C, D), this._$El = null;
    }
  }
  _$AK(A, g) {
    var I;
    const n = this.constructor, C = n._$Ev.get(A);
    if (C !== void 0 && this._$El !== C) {
      const D = n.getPropertyOptions(C), t = typeof D.converter == "function" ? { fromAttribute: D.converter } : ((I = D.converter) === null || I === void 0 ? void 0 : I.fromAttribute) !== void 0 ? D.converter : uA;
      this._$El = C, this[C] = t.fromAttribute(g, D.type), this._$El = null;
    }
  }
  requestUpdate(A, g, I) {
    let n = !0;
    A !== void 0 && (((I = I || this.constructor.getPropertyOptions(A)).hasChanged || _A)(this[A], g) ? (this._$AL.has(A) || this._$AL.set(A, g), I.reflect === !0 && this._$El !== A && (this._$EC === void 0 && (this._$EC = /* @__PURE__ */ new Map()), this._$EC.set(A, I))) : n = !1), !this.isUpdatePending && n && (this._$E_ = this._$Ej());
  }
  async _$Ej() {
    this.isUpdatePending = !0;
    try {
      await this._$E_;
    } catch (g) {
      Promise.reject(g);
    }
    const A = this.scheduleUpdate();
    return A != null && await A, !this.isUpdatePending;
  }
  scheduleUpdate() {
    return this.performUpdate();
  }
  performUpdate() {
    var A;
    if (!this.isUpdatePending) return;
    this.hasUpdated, this._$Ei && (this._$Ei.forEach((n, C) => this[C] = n), this._$Ei = void 0);
    let g = !1;
    const I = this._$AL;
    try {
      g = this.shouldUpdate(I), g ? (this.willUpdate(I), (A = this._$ES) === null || A === void 0 || A.forEach((n) => {
        var C;
        return (C = n.hostUpdate) === null || C === void 0 ? void 0 : C.call(n);
      }), this.update(I)) : this._$Ek();
    } catch (n) {
      throw g = !1, this._$Ek(), n;
    }
    g && this._$AE(I);
  }
  willUpdate(A) {
  }
  _$AE(A) {
    var g;
    (g = this._$ES) === null || g === void 0 || g.forEach((I) => {
      var n;
      return (n = I.hostUpdated) === null || n === void 0 ? void 0 : n.call(I);
    }), this.hasUpdated || (this.hasUpdated = !0, this.firstUpdated(A)), this.updated(A);
  }
  _$Ek() {
    this._$AL = /* @__PURE__ */ new Map(), this.isUpdatePending = !1;
  }
  get updateComplete() {
    return this.getUpdateComplete();
  }
  getUpdateComplete() {
    return this._$E_;
  }
  shouldUpdate(A) {
    return !0;
  }
  update(A) {
    this._$EC !== void 0 && (this._$EC.forEach((g, I) => this._$EO(I, this[I], g)), this._$EC = void 0), this._$Ek();
  }
  updated(A) {
  }
  firstUpdated(A) {
  }
};
gA[pA] = !0, gA.elementProperties = /* @__PURE__ */ new Map(), gA.elementStyles = [], gA.shadowRootOptions = { mode: "open" }, YA == null || YA({ ReactiveElement: gA }), ((fA = rA.reactiveElementVersions) !== null && fA !== void 0 ? fA : rA.reactiveElementVersions = []).push("1.6.3");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
var dA;
const EA = window, CA = EA.trustedTypes, WA = CA ? CA.createPolicy("lit-html", { createHTML: (e) => e }) : void 0, cA = "$lit$", X = `lit$${(Math.random() + "").slice(9)}$`, $A = "?" + X, lg = `<${$A}>`, _ = document, iA = () => _.createComment(""), lA = (e) => e === null || typeof e != "object" && typeof e != "function", Ag = Array.isArray, ag = (e) => Ag(e) || typeof (e == null ? void 0 : e[Symbol.iterator]) == "function", mA = `[ 	
\f\r]`, hA = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, bA = /-->/g, MA = />/g, z = RegExp(`>|${mA}(?:([^\\s"'>=/]+)(${mA}*=${mA}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), ZA = /'/g, GA = /"/g, gg = /^(?:script|style|textarea|title)$/i, og = (e) => (A, ...g) => ({ _$litType$: e, strings: A, values: g }), d = og(1), nA = Symbol.for("lit-noChange"), j = Symbol.for("lit-nothing"), FA = /* @__PURE__ */ new WeakMap(), N = _.createTreeWalker(_, 129, null, !1);
function Ig(e, A) {
  if (!Array.isArray(e) || !e.hasOwnProperty("raw")) throw Error("invalid template strings array");
  return WA !== void 0 ? WA.createHTML(A) : A;
}
const Qg = (e, A) => {
  const g = e.length - 1, I = [];
  let n, C = A === 2 ? "<svg>" : "", D = hA;
  for (let t = 0; t < g; t++) {
    const h = e[t];
    let i, a, r = -1, Q = 0;
    for (; Q < h.length && (D.lastIndex = Q, a = D.exec(h), a !== null); ) Q = D.lastIndex, D === hA ? a[1] === "!--" ? D = bA : a[1] !== void 0 ? D = MA : a[2] !== void 0 ? (gg.test(a[2]) && (n = RegExp("</" + a[2], "g")), D = z) : a[3] !== void 0 && (D = z) : D === z ? a[0] === ">" ? (D = n ?? hA, r = -1) : a[1] === void 0 ? r = -2 : (r = D.lastIndex - a[2].length, i = a[1], D = a[3] === void 0 ? z : a[3] === '"' ? GA : ZA) : D === GA || D === ZA ? D = z : D === bA || D === MA ? D = hA : (D = z, n = void 0);
    const l = D === z && e[t + 1].startsWith("/>") ? " " : "";
    C += D === hA ? h + lg : r >= 0 ? (I.push(i), h.slice(0, r) + cA + h.slice(r) + X + l) : h + X + (r === -2 ? (I.push(void 0), t) : l);
  }
  return [Ig(e, C + (e[g] || "<?>") + (A === 2 ? "</svg>" : "")), I];
};
class aA {
  constructor({ strings: A, _$litType$: g }, I) {
    let n;
    this.parts = [];
    let C = 0, D = 0;
    const t = A.length - 1, h = this.parts, [i, a] = Qg(A, g);
    if (this.el = aA.createElement(i, I), N.currentNode = this.el.content, g === 2) {
      const r = this.el.content, Q = r.firstChild;
      Q.remove(), r.append(...Q.childNodes);
    }
    for (; (n = N.nextNode()) !== null && h.length < t; ) {
      if (n.nodeType === 1) {
        if (n.hasAttributes()) {
          const r = [];
          for (const Q of n.getAttributeNames()) if (Q.endsWith(cA) || Q.startsWith(X)) {
            const l = a[D++];
            if (r.push(Q), l !== void 0) {
              const m = n.getAttribute(l.toLowerCase() + cA).split(X), R = /([.?@])?(.*)/.exec(l);
              h.push({ type: 1, index: C, name: R[2], strings: m, ctor: R[1] === "." ? rg : R[1] === "?" ? Sg : R[1] === "@" ? fg : SA });
            } else h.push({ type: 6, index: C });
          }
          for (const Q of r) n.removeAttribute(Q);
        }
        if (gg.test(n.tagName)) {
          const r = n.textContent.split(X), Q = r.length - 1;
          if (Q > 0) {
            n.textContent = CA ? CA.emptyScript : "";
            for (let l = 0; l < Q; l++) n.append(r[l], iA()), N.nextNode(), h.push({ type: 2, index: ++C });
            n.append(r[Q], iA());
          }
        }
      } else if (n.nodeType === 8) if (n.data === $A) h.push({ type: 2, index: C });
      else {
        let r = -1;
        for (; (r = n.data.indexOf(X, r + 1)) !== -1; ) h.push({ type: 7, index: C }), r += X.length - 1;
      }
      C++;
    }
  }
  static createElement(A, g) {
    const I = _.createElement("template");
    return I.innerHTML = A, I;
  }
}
function DA(e, A, g = e, I) {
  var n, C, D, t;
  if (A === nA) return A;
  let h = I !== void 0 ? (n = g._$Co) === null || n === void 0 ? void 0 : n[I] : g._$Cl;
  const i = lA(A) ? void 0 : A._$litDirective$;
  return (h == null ? void 0 : h.constructor) !== i && ((C = h == null ? void 0 : h._$AO) === null || C === void 0 || C.call(h, !1), i === void 0 ? h = void 0 : (h = new i(e), h._$AT(e, g, I)), I !== void 0 ? ((D = (t = g)._$Co) !== null && D !== void 0 ? D : t._$Co = [])[I] = h : g._$Cl = h), h !== void 0 && (A = DA(e, h._$AS(e, A.values), h, I)), A;
}
class sg {
  constructor(A, g) {
    this._$AV = [], this._$AN = void 0, this._$AD = A, this._$AM = g;
  }
  get parentNode() {
    return this._$AM.parentNode;
  }
  get _$AU() {
    return this._$AM._$AU;
  }
  u(A) {
    var g;
    const { el: { content: I }, parts: n } = this._$AD, C = ((g = A == null ? void 0 : A.creationScope) !== null && g !== void 0 ? g : _).importNode(I, !0);
    N.currentNode = C;
    let D = N.nextNode(), t = 0, h = 0, i = n[0];
    for (; i !== void 0; ) {
      if (t === i.index) {
        let a;
        i.type === 2 ? a = new oA(D, D.nextSibling, this, A) : i.type === 1 ? a = new i.ctor(D, i.name, i.strings, this, A) : i.type === 6 && (a = new Ug(D, this, A)), this._$AV.push(a), i = n[++h];
      }
      t !== (i == null ? void 0 : i.index) && (D = N.nextNode(), t++);
    }
    return N.currentNode = _, C;
  }
  v(A) {
    let g = 0;
    for (const I of this._$AV) I !== void 0 && (I.strings !== void 0 ? (I._$AI(A, I, g), g += I.strings.length - 2) : I._$AI(A[g])), g++;
  }
}
class oA {
  constructor(A, g, I, n) {
    var C;
    this.type = 2, this._$AH = j, this._$AN = void 0, this._$AA = A, this._$AB = g, this._$AM = I, this.options = n, this._$Cp = (C = n == null ? void 0 : n.isConnected) === null || C === void 0 || C;
  }
  get _$AU() {
    var A, g;
    return (g = (A = this._$AM) === null || A === void 0 ? void 0 : A._$AU) !== null && g !== void 0 ? g : this._$Cp;
  }
  get parentNode() {
    let A = this._$AA.parentNode;
    const g = this._$AM;
    return g !== void 0 && (A == null ? void 0 : A.nodeType) === 11 && (A = g.parentNode), A;
  }
  get startNode() {
    return this._$AA;
  }
  get endNode() {
    return this._$AB;
  }
  _$AI(A, g = this) {
    A = DA(this, A, g), lA(A) ? A === j || A == null || A === "" ? (this._$AH !== j && this._$AR(), this._$AH = j) : A !== this._$AH && A !== nA && this._(A) : A._$litType$ !== void 0 ? this.g(A) : A.nodeType !== void 0 ? this.$(A) : ag(A) ? this.T(A) : this._(A);
  }
  k(A) {
    return this._$AA.parentNode.insertBefore(A, this._$AB);
  }
  $(A) {
    this._$AH !== A && (this._$AR(), this._$AH = this.k(A));
  }
  _(A) {
    this._$AH !== j && lA(this._$AH) ? this._$AA.nextSibling.data = A : this.$(_.createTextNode(A)), this._$AH = A;
  }
  g(A) {
    var g;
    const { values: I, _$litType$: n } = A, C = typeof n == "number" ? this._$AC(A) : (n.el === void 0 && (n.el = aA.createElement(Ig(n.h, n.h[0]), this.options)), n);
    if (((g = this._$AH) === null || g === void 0 ? void 0 : g._$AD) === C) this._$AH.v(I);
    else {
      const D = new sg(C, this), t = D.u(this.options);
      D.v(I), this.$(t), this._$AH = D;
    }
  }
  _$AC(A) {
    let g = FA.get(A.strings);
    return g === void 0 && FA.set(A.strings, g = new aA(A)), g;
  }
  T(A) {
    Ag(this._$AH) || (this._$AH = [], this._$AR());
    const g = this._$AH;
    let I, n = 0;
    for (const C of A) n === g.length ? g.push(I = new oA(this.k(iA()), this.k(iA()), this, this.options)) : I = g[n], I._$AI(C), n++;
    n < g.length && (this._$AR(I && I._$AB.nextSibling, n), g.length = n);
  }
  _$AR(A = this._$AA.nextSibling, g) {
    var I;
    for ((I = this._$AP) === null || I === void 0 || I.call(this, !1, !0, g); A && A !== this._$AB; ) {
      const n = A.nextSibling;
      A.remove(), A = n;
    }
  }
  setConnected(A) {
    var g;
    this._$AM === void 0 && (this._$Cp = A, (g = this._$AP) === null || g === void 0 || g.call(this, A));
  }
}
class SA {
  constructor(A, g, I, n, C) {
    this.type = 1, this._$AH = j, this._$AN = void 0, this.element = A, this.name = g, this._$AM = n, this.options = C, I.length > 2 || I[0] !== "" || I[1] !== "" ? (this._$AH = Array(I.length - 1).fill(new String()), this.strings = I) : this._$AH = j;
  }
  get tagName() {
    return this.element.tagName;
  }
  get _$AU() {
    return this._$AM._$AU;
  }
  _$AI(A, g = this, I, n) {
    const C = this.strings;
    let D = !1;
    if (C === void 0) A = DA(this, A, g, 0), D = !lA(A) || A !== this._$AH && A !== nA, D && (this._$AH = A);
    else {
      const t = A;
      let h, i;
      for (A = C[0], h = 0; h < C.length - 1; h++) i = DA(this, t[I + h], g, h), i === nA && (i = this._$AH[h]), D || (D = !lA(i) || i !== this._$AH[h]), i === j ? A = j : A !== j && (A += (i ?? "") + C[h + 1]), this._$AH[h] = i;
    }
    D && !n && this.j(A);
  }
  j(A) {
    A === j ? this.element.removeAttribute(this.name) : this.element.setAttribute(this.name, A ?? "");
  }
}
class rg extends SA {
  constructor() {
    super(...arguments), this.type = 3;
  }
  j(A) {
    this.element[this.name] = A === j ? void 0 : A;
  }
}
const Eg = CA ? CA.emptyScript : "";
class Sg extends SA {
  constructor() {
    super(...arguments), this.type = 4;
  }
  j(A) {
    A && A !== j ? this.element.setAttribute(this.name, Eg) : this.element.removeAttribute(this.name);
  }
}
class fg extends SA {
  constructor(A, g, I, n, C) {
    super(A, g, I, n, C), this.type = 5;
  }
  _$AI(A, g = this) {
    var I;
    if ((A = (I = DA(this, A, g, 0)) !== null && I !== void 0 ? I : j) === nA) return;
    const n = this._$AH, C = A === j && n !== j || A.capture !== n.capture || A.once !== n.once || A.passive !== n.passive, D = A !== j && (n === j || C);
    C && this.element.removeEventListener(this.name, this, n), D && this.element.addEventListener(this.name, this, A), this._$AH = A;
  }
  handleEvent(A) {
    var g, I;
    typeof this._$AH == "function" ? this._$AH.call((I = (g = this.options) === null || g === void 0 ? void 0 : g.host) !== null && I !== void 0 ? I : this.element, A) : this._$AH.handleEvent(A);
  }
}
class Ug {
  constructor(A, g, I) {
    this.element = A, this.type = 6, this._$AN = void 0, this._$AM = g, this.options = I;
  }
  get _$AU() {
    return this._$AM._$AU;
  }
  _$AI(A) {
    DA(this, A);
  }
}
const KA = EA.litHtmlPolyfillSupport;
KA == null || KA(aA, oA), ((dA = EA.litHtmlVersions) !== null && dA !== void 0 ? dA : EA.litHtmlVersions = []).push("2.8.0");
const dg = (e, A, g) => {
  var I, n;
  const C = (I = g == null ? void 0 : g.renderBefore) !== null && I !== void 0 ? I : A;
  let D = C._$litPart$;
  if (D === void 0) {
    const t = (n = g == null ? void 0 : g.renderBefore) !== null && n !== void 0 ? n : null;
    C._$litPart$ = D = new oA(A.insertBefore(iA(), t), t, void 0, g ?? {});
  }
  return D._$AI(e), D;
};
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
var xA, BA;
class IA extends gA {
  constructor() {
    super(...arguments), this.renderOptions = { host: this }, this._$Do = void 0;
  }
  createRenderRoot() {
    var A, g;
    const I = super.createRenderRoot();
    return (A = (g = this.renderOptions).renderBefore) !== null && A !== void 0 || (g.renderBefore = I.firstChild), I;
  }
  update(A) {
    const g = this.render();
    this.hasUpdated || (this.renderOptions.isConnected = this.isConnected), super.update(A), this._$Do = dg(g, this.renderRoot, this.renderOptions);
  }
  connectedCallback() {
    var A;
    super.connectedCallback(), (A = this._$Do) === null || A === void 0 || A.setConnected(!0);
  }
  disconnectedCallback() {
    var A;
    super.disconnectedCallback(), (A = this._$Do) === null || A === void 0 || A.setConnected(!1);
  }
  render() {
    return nA;
  }
}
IA.finalized = !0, IA._$litElement$ = !0, (xA = globalThis.litElementHydrateSupport) === null || xA === void 0 || xA.call(globalThis, { LitElement: IA });
const LA = globalThis.litElementPolyfillSupport;
LA == null || LA({ LitElement: IA });
((BA = globalThis.litElementVersions) !== null && BA !== void 0 ? BA : globalThis.litElementVersions = []).push("3.3.3");
const mg = "data:image/png;base64,", xg = "data:image/png;base64,", Bg = "data:image/png;base64,", ug = "data:image/png;base64,", pg = "data:image/png;base64,", cg = "data:image/png;base64,", yg = "data:image/png;base64,", Hg = "data:image/png;base64,", jg = "data:image/png;base64,", Rg = "data:image/png;base64,", wg = "data:image/png;base64,", Jg = "data:image/png;base64,", vg = "data:image/png;base64,", kg = "data:image/png;base64,", Pg = "data:image/png;base64,", Og = "data:image/png;base64,", Yg = "data:image/png;base64,", Wg = "data:image/png;base64,", bg = "data:image/png;base64,", Mg = "data:image/png;base64,", Zg = "data:image/png;base64,", Gg = "data:image/png;base64,", Fg = "data:image/png;base64,", Kg = "data:image/png;base64,", Lg = "data:image/png;base64,", Tg = "data:image/png;base64,", Vg = "data:image/png;base64,", Xg = "data:image/png;base64,", qg = "data:image/png;base64,", zg = "data:image/png;base64,", Ng = "data:image/png;base64,", _g = "data:image/png;base64,", $g = "data:image/png;base64,", AI = "data:image/png;base64,", gI = "data:image/png;base64,", II = "data:image/png;base64,", CI = "data:image/png;base64,", nI = "data:image/png;base64,", DI = "data:image/png;base64,", eI = "data:image/png;base64,", hI = "data:image/png;base64,", tI = "data:image/png;base64,", iI = "data:image/png;base64,", lI = "data:image/png;base64,", aI = "data:image/png;base64,", oI = "data:image/png;base64,", QI = "data:image/png;base64,", sI = "data:image/png;base64,", rI = "data:image/png;base64,", EI = "data:image/png;base64,", SI = "data:image/png;base64,", fI = "data:image/png;base64,", UI = "data:image/png;base64,", dI = "data:image/png;base64,", mI = "data:image/png;base64,", xI = "data:image/png;base64,", BI = "data:image/png;base64,", uI = "data:image/png;base64,", pI = "data:image/png;base64,", cI = "data:image/png;base64,", yI = "data:image/png;base64,", HI = "data:image/png;base64,", jI = "data:image/png;base64,", RI = "data:image/png;base64,", wI = "data:image/png;base64,", JI = "data:image/png;base64,", vI = "data:image/png;base64,", kI = "data:image/png;base64,", PI = "data:image/png;base64,", OI = "data:image/png;base64,", YI = "data:image/png;base64,", WI = "data:image/png;base64,", bI = "data:image/png;base64,", MI = "data:image/png;base64,", ZI = "data:image/png;base64,", GI = "data:image/png;base64,", FI = "data:image/png;base64,", KI = "data:image/png;base64,", LI = "data:image/png;base64,", TI = "data:image/png;base64,", VI = "data:image/png;base64,", XI = "data:image/png;base64,", qI = "data:image/png;base64,", zI = "data:image/png;base64,", NI = "data:image/png;base64,", _I = "data:image/png;base64,", $I = "data:image/png;base64,", AC = "data:image/png;base64,", gC = "data:image/png;base64,", IC = "data:image/png;base64,", CC = "data:image/png;base64,", nC = "data:image/png;base64,", DC = "data:image/png;base64,", eC = "data:image/png;base64,", hC = "data:image/png;base64,", tC = "data:image/png;base64,", iC = "data:image/png;base64,", lC = "data:image/png;base64,", aC = "data:image/png;base64,", oC = "data:image/png;base64,", QC = "data:image/png;base64,", sC = "data:image/png;base64,", TA = {
  // global
  "-1_png": mg,
  "0_png": xg,
  "1_png": Bg,
  "2_png": ug,
  "3_png": pg,
  "4_png": cg,
  "5_png": yg,
  "6_png": Hg,
  // alder
  "alder_-1_png": jg,
  alder_0_png: Rg,
  alder_1_png: wg,
  alder_2_png: Jg,
  alder_3_png: vg,
  alder_4_png: kg,
  alder_5_png: Pg,
  alder_6_png: Og,
  // elm
  elm_0_png: Yg,
  elm_1_png: Wg,
  elm_2_png: bg,
  elm_3_png: Mg,
  elm_4_png: Zg,
  elm_5_png: Gg,
  elm_6_png: Fg,
  // birch
  "birch_-1_png": Kg,
  birch_0_png: Lg,
  birch_1_png: Tg,
  birch_2_png: Vg,
  birch_3_png: Xg,
  birch_4_png: qg,
  birch_5_png: zg,
  birch_6_png: Ng,
  // beech
  beech_0_png: _g,
  beech_1_png: $g,
  beech_2_png: AI,
  beech_3_png: gI,
  beech_4_png: II,
  beech_5_png: CI,
  beech_6_png: nI,
  // oak
  "oak_-1_png": DI,
  oak_0_png: eI,
  oak_1_png: hI,
  oak_2_png: tI,
  oak_3_png: iI,
  oak_4_png: lI,
  oak_5_png: aI,
  oak_6_png: oI,
  // mugwort
  "mugwort_-1_png": QI,
  mugwort_0_png: sI,
  mugwort_1_png: rI,
  mugwort_2_png: EI,
  mugwort_3_png: SI,
  mugwort_4_png: fI,
  mugwort_5_png: UI,
  mugwort_6_png: dI,
  // grass
  "grass_-1_png": mI,
  grass_0_png: xI,
  grass_1_png: BI,
  grass_2_png: uI,
  grass_3_png: pI,
  grass_4_png: cI,
  grass_5_png: yI,
  grass_6_png: HI,
  // hazel
  "hazel_-1_png": jI,
  hazel_0_png: RI,
  hazel_1_png: wI,
  hazel_2_png: JI,
  hazel_3_png: vI,
  hazel_4_png: kI,
  hazel_5_png: PI,
  hazel_6_png: OI,
  // ragweed
  "ragweed_-1_png": YI,
  ragweed_0_png: WI,
  ragweed_1_png: bI,
  ragweed_2_png: MI,
  ragweed_3_png: ZI,
  ragweed_4_png: GI,
  ragweed_5_png: FI,
  ragweed_6_png: KI,
  // willow
  "willow_-1_png": LI,
  willow_0_png: TI,
  willow_1_png: VI,
  willow_2_png: XI,
  willow_3_png: qI,
  willow_4_png: zI,
  willow_5_png: NI,
  willow_6_png: _I,
  // ash
  "ash_-1_png": $I,
  ash_0_png: AC,
  ash_1_png: gC,
  ash_2_png: IC,
  ash_3_png: CC,
  ash_4_png: nC,
  ash_5_png: DC,
  ash_6_png: eC,
  // rye
  "rye_-1_png": hC,
  rye_0_png: tC,
  rye_1_png: iC,
  rye_2_png: lC,
  rye_3_png: aC,
  rye_4_png: oC,
  rye_5_png: QC,
  rye_6_png: sC
}, rC = {
  "card.allergen.alder": "Erle",
  "card.allergen.ash": "Esche",
  "card.allergen.beech": "Buche",
  "card.allergen.birch": "Birke",
  "card.allergen.cypress": "Zypresse",
  "card.allergen.elm": "Ulme",
  "card.allergen.grass": "Gräser",
  "card.allergen.hazel": "Hasel",
  "card.allergen.lime": "Linde",
  "card.allergen.mold_spores": "Schimmelsporen",
  "card.allergen.mugwort": "Beifuß",
  "card.allergen.nettle_and_pellitory": "Nessel & Parietarie",
  "card.allergen.oak": "Eiche",
  "card.allergen.olive": "Olive",
  "card.allergen.plane": "Platane",
  "card.allergen.ragweed": "Ambrosia",
  "card.allergen.rye": "Roggen",
  "card.allergen.willow": "Weide",
  "card.days.0": "Heute",
  "card.days.1": "Morgen",
  "card.days.2": "Übermorgen",
  "card.error": "Keine Pollensensoren gefunden. Haben Sie die richtige Integration installiert und eine Region in der Kartenkonfiguration ausgewählt?",
  "card.header_prefix": "Pollenprognose für",
  "card.integration.dwd": "DWD Pollenflug",
  "card.integration.pp": "PollenPrognos",
  "card.levels.0": "keine Belastung",
  "card.levels.1": "keine bis geringe Belastung",
  "card.levels.2": "geringe Belastung",
  "card.levels.3": "geringe bis mittlere Belastung",
  "card.levels.4": "mittlere Belastung",
  "card.levels.5": "mittlere bis hohe Belastung",
  "card.levels.6": "hohe Belastung",
  "card.no_information": "(Keine Information)",
  "editor.allergens": "Allergene",
  "editor.allergens_abbreviated": "Allergene abkürzen",
  "editor.city": "Stadt",
  "editor.days_abbreviated": "Wochentage abkürzen",
  "editor.days_boldfaced": "Wochentage fett",
  "editor.days_relative": "Relative Tage (heute/morgen)",
  "editor.days_to_show": "Anzahl Tage:",
  "editor.days_uppercase": "Wochentage groß",
  "editor.debug": "Debug",
  "editor.integration": "Integration",
  "editor.integration.dwd": "DWD Pollenflug",
  "editor.integration.pp": "PollenPrognos",
  "editor.locale": "Locale",
  "editor.minimal": "Minimal",
  "editor.no_information": "Keine Information",
  "editor.phrases": "Phrasen",
  "editor.phrases_apply": "Übernehmen",
  "editor.phrases_days": "Relative Tage",
  "editor.phrases_days.0": "Heute",
  "editor.phrases_days.1": "Morgen",
  "editor.phrases_days.2": "Übermorgen",
  "editor.phrases_full": "Allergene",
  "editor.phrases_full.alder": "Erle",
  "editor.phrases_full.ash": "Esche",
  "editor.phrases_full.beech": "Buche",
  "editor.phrases_full.birch": "Birke",
  "editor.phrases_full.cypress": "Zypresse",
  "editor.phrases_full.elm": "Ulme",
  "editor.phrases_full.grass": "Gräser",
  "editor.phrases_full.hazel": "Hasel",
  "editor.phrases_full.lime": "Linde",
  "editor.phrases_full.mold_spores": "Schimmelsporen",
  "editor.phrases_full.mugwort": "Beifuß",
  "editor.phrases_full.nettle_and_pellitory": "Nessel & Parietarie",
  "editor.phrases_full.oak": "Eiche",
  "editor.phrases_full.olive": "Olive",
  "editor.phrases_full.plane": "Platane",
  "editor.phrases_full.ragweed": "Ambrosia",
  "editor.phrases_full.rye": "Roggen",
  "editor.phrases_full.willow": "Weide",
  "editor.phrases_levels": "Allergenstufen",
  "editor.phrases_levels.0": "keine Belastung",
  "editor.phrases_levels.1": "keine bis geringe Belastung",
  "editor.phrases_levels.2": "geringe Belastung",
  "editor.phrases_levels.3": "geringe bis mittlere Belastung",
  "editor.phrases_levels.4": "mittlere Belastung",
  "editor.phrases_levels.5": "mittlere bis hohe Belastung",
  "editor.phrases_levels.6": "hohe Belastung",
  "editor.phrases_short": "Allergene, kurz",
  "editor.phrases_short.alder": "Erle",
  "editor.phrases_short.ash": "Esche",
  "editor.phrases_short.birch": "Birke",
  "editor.phrases_short.cypress": "Zyp.",
  "editor.phrases_short.elm": "Ulme",
  "editor.phrases_short.grass": "Gräs",
  "editor.phrases_short.hazel": "Hasel",
  "editor.phrases_short.lime": "Linde",
  "editor.phrases_short.mold_spores": "Schimmel",
  "editor.phrases_short.mugwort": "Beifu",
  "editor.phrases_short.nettle_and_pellitory": "Nessel",
  "editor.phrases_short.oak": "Eiche",
  "editor.phrases_short.olive": "Olive",
  "editor.phrases_short.plane": "Plat.",
  "editor.phrases_short.ragweed": "Ambro",
  "editor.phrases_short.rye": "Roggn",
  "editor.phrases_short.willow": "Weide",
  "editor.phrases_translate_all": "Alle übersetzen",
  "editor.pollen_threshold": "Schwelle:",
  "editor.preset_reset_all": "Alles zurücksetzen",
  "editor.region_id": "Region ID",
  "editor.show_empty_days": "Leere Tage anzeigen",
  "editor.show_text_allergen": "Allergentext anzeigen",
  "editor.show_value_numeric": "Wert als Zahl anzeigen",
  "editor.show_value_numeric_in_circle": "Numerischen Wert im Kreis anzeigen",
  "editor.show_value_text": "Wert als Text anzeigen",
  "editor.sort": "Sortierung",
  "editor.title": "Titel (leer = Auto)",
  "card.integration.peu": "Polleninformation EU"
}, EC = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: rC
}, Symbol.toStringTag, { value: "Module" })), SC = {
  "card.allergen.alder": "Alder",
  "card.allergen.ash": "Ash",
  "card.allergen.beech": "Beech",
  "card.allergen.birch": "Birch",
  "card.allergen.cypress": "Cypress",
  "card.allergen.elm": "Elm",
  "card.allergen.grass": "Grass",
  "card.allergen.hazel": "Hazel",
  "card.allergen.lime": "Lime",
  "card.allergen.mold_spores": "Mold spores",
  "card.allergen.mugwort": "Mugwort",
  "card.allergen.nettle_and_pellitory": "Nettle and pellitory",
  "card.allergen.oak": "Oak",
  "card.allergen.olive": "Olive",
  "card.allergen.plane": "Plane",
  "card.allergen.ragweed": "Ragweed",
  "card.allergen.rye": "Rye",
  "card.allergen.willow": "Willow",
  "card.days.0": "Today",
  "card.days.1": "Tomorrow",
  "card.days.2": "Day after tomorrow",
  "card.error": "No pollen sensors found. Have you installed the correct integration and selected a region in the card configuration?",
  "card.header_prefix": "Pollen forecast for",
  "card.integration.dwd": "DWD Pollenflug",
  "card.integration.pp": "PollenPrognos",
  "card.levels.0": "No pollen",
  "card.levels.1": "Low levels",
  "card.levels.2": "Low–moderate levels",
  "card.levels.3": "Moderate levels",
  "card.levels.4": "Moderate–high levels",
  "card.levels.5": "High levels",
  "card.levels.6": "Very high levels",
  "card.no_information": "(No information)",
  "editor.allergens": "Allergens",
  "editor.allergens_abbreviated": "Abbreviate allergens",
  "editor.city": "City",
  "editor.days_abbreviated": "Abbreviate weekdays",
  "editor.days_boldfaced": "Bold weekdays",
  "editor.days_relative": "Relative days (today/tomorrow)",
  "editor.days_to_show": "Days to show:",
  "editor.days_uppercase": "Uppercase weekdays",
  "editor.debug": "Debug",
  "editor.integration": "Integration",
  "editor.integration.dwd": "DWD Pollenflug",
  "editor.integration.pp": "PollenPrognos",
  "editor.locale": "Locale",
  "editor.minimal": "Minimal",
  "editor.no_information": "No information",
  "editor.phrases": "Phrases",
  "editor.phrases_apply": "Apply",
  "editor.phrases_days": "Relative days",
  "editor.phrases_days.0": "Today",
  "editor.phrases_days.1": "Tomorrow",
  "editor.phrases_days.2": "Day after tomorrow",
  "editor.phrases_full": "Allergens",
  "editor.phrases_full.alder": "Alder",
  "editor.phrases_full.ash": "Ash",
  "editor.phrases_full.beech": "Beech",
  "editor.phrases_full.birch": "Birch",
  "editor.phrases_full.cypress": "Cypress",
  "editor.phrases_full.elm": "Elm",
  "editor.phrases_full.grass": "Grass",
  "editor.phrases_full.hazel": "Hazel",
  "editor.phrases_full.lime": "Lime",
  "editor.phrases_full.mold_spores": "Mold spores",
  "editor.phrases_full.mugwort": "Mugwort",
  "editor.phrases_full.nettle_and_pellitory": "Nettle and pellitory",
  "editor.phrases_full.oak": "Oak",
  "editor.phrases_full.olive": "Olive",
  "editor.phrases_full.plane": "Plane",
  "editor.phrases_full.ragweed": "Ragweed",
  "editor.phrases_full.rye": "Rye",
  "editor.phrases_full.willow": "Willow",
  "editor.phrases_levels": "Allergen levels",
  "editor.phrases_levels.0": "No pollen",
  "editor.phrases_levels.1": "Low levels",
  "editor.phrases_levels.2": "Low–moderate levels",
  "editor.phrases_levels.3": "Moderate levels",
  "editor.phrases_levels.4": "Moderate–high levels",
  "editor.phrases_levels.5": "High levels",
  "editor.phrases_levels.6": "Very high levels",
  "editor.phrases_short": "Allergens, short",
  "editor.phrases_short.alder": "Aldr",
  "editor.phrases_short.ash": "Ash",
  "editor.phrases_short.birch": "Birch",
  "editor.phrases_short.cypress": "Cypress",
  "editor.phrases_short.elm": "Elm",
  "editor.phrases_short.grass": "Grass",
  "editor.phrases_short.hazel": "Hazel",
  "editor.phrases_short.lime": "Lime",
  "editor.phrases_short.mold_spores": "Mold",
  "editor.phrases_short.mugwort": "Mgwrt",
  "editor.phrases_short.nettle_and_pellitory": "Nettle",
  "editor.phrases_short.oak": "Oak",
  "editor.phrases_short.olive": "Olive",
  "editor.phrases_short.plane": "Plane",
  "editor.phrases_short.ragweed": "Rgwd",
  "editor.phrases_short.rye": "Rye",
  "editor.phrases_short.willow": "Wllw",
  "editor.phrases_translate_all": "Translate all",
  "editor.pollen_threshold": "Threshold:",
  "editor.preset_reset_all": "Reset all settings",
  "editor.region_id": "Region ID",
  "editor.show_empty_days": "Show empty days",
  "editor.show_text_allergen": "Show text, allergen",
  "editor.show_value_numeric": "Show value, numeric",
  "editor.show_value_numeric_in_circle": "Show numeric value in the circles",
  "editor.show_value_text": "Show value, text",
  "editor.sort": "Sort order",
  "editor.title": "Title (empty = auto)",
  "card.integration.peu": "Polleninformation EU"
}, fC = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: SC
}, Symbol.toStringTag, { value: "Module" })), UC = {
  "card.allergen.alder": "Al",
  "card.allergen.ash": "Asp",
  "card.allergen.beech": "Bok",
  "card.allergen.birch": "Björk",
  "card.allergen.cypress": "Cypress",
  "card.allergen.elm": "Alm",
  "card.allergen.grass": "Gräs",
  "card.allergen.hazel": "Hassel",
  "card.allergen.lime": "Lind",
  "card.allergen.mold_spores": "Mögelsporer",
  "card.allergen.mugwort": "Gråbo",
  "card.allergen.nettle_and_pellitory": "Nässla & parietaria",
  "card.allergen.oak": "Ek",
  "card.allergen.olive": "Oliv",
  "card.allergen.plane": "Platan",
  "card.allergen.ragweed": "Malörtsambrosia",
  "card.allergen.rye": "Råg",
  "card.allergen.willow": "Sälg och viden",
  "card.days.0": "Idag",
  "card.days.1": "Imorgon",
  "card.days.2": "I övermorgon",
  "card.error": "Inga pollen-sensorer hittades. Har du installerat rätt integration och valt region i kortets konfiguration?",
  "card.header_prefix": "Pollenprognos för",
  "card.integration.dwd": "DWD Pollenflug",
  "card.integration.pp": "PollenPrognos",
  "card.levels.0": "Ingen pollen",
  "card.levels.1": "Låga halter",
  "card.levels.2": "Låga–måttliga halter",
  "card.levels.3": "Måttliga halter",
  "card.levels.4": "Måttliga–höga halter",
  "card.levels.5": "Höga halter",
  "card.levels.6": "Mycket höga halter",
  "card.no_information": "(Ingen information)",
  "editor.allergens": "Allergener",
  "editor.allergens_abbreviated": "Förkorta allergener",
  "editor.city": "Stad",
  "editor.days_abbreviated": "Förkorta veckodagar",
  "editor.days_boldfaced": "Fetstil veckodagar",
  "editor.days_relative": "Relativa dagar (idag/imorgon)",
  "editor.days_to_show": "Antal dagar:",
  "editor.days_uppercase": "Versaler veckodagar",
  "editor.debug": "Debug",
  "editor.integration": "Integration",
  "editor.integration.dwd": "DWD Pollenflug",
  "editor.integration.pp": "PollenPrognos",
  "editor.locale": "Locale",
  "editor.minimal": "Minimal",
  "editor.no_information": "Ingen information",
  "editor.phrases": "Fraser",
  "editor.phrases_apply": "Utför",
  "editor.phrases_days": "Relativa dagar",
  "editor.phrases_days.0": "Idag",
  "editor.phrases_days.1": "Imorgon",
  "editor.phrases_days.2": "I övermorgon",
  "editor.phrases_full": "Allergener",
  "editor.phrases_full.alder": "Al",
  "editor.phrases_full.ash": "Asp",
  "editor.phrases_full.beech": "Bok",
  "editor.phrases_full.birch": "Björk",
  "editor.phrases_full.cypress": "Cypress",
  "editor.phrases_full.grass": "Gräs",
  "editor.phrases_full.hazel": "Hassel",
  "editor.phrases_full.lime": "Lind",
  "editor.phrases_full.mold_spores": "Mögelsporer",
  "editor.phrases_full.mugwort": "Gråbo",
  "editor.phrases_full.nettle_and_pellitory": "Nässla & parietaria",
  "editor.phrases_full.oak": "Ek",
  "editor.phrases_full.olive": "Oliv",
  "editor.phrases_full.plane": "Platan",
  "editor.phrases_full.ragweed": "Malörtsambrosia",
  "editor.phrases_full.rye": "Råg",
  "editor.phrases_full.willow": "Sälg och viden",
  "editor.phrases_levels": "Allergennivåer",
  "editor.phrases_levels.0": "Ingen pollen",
  "editor.phrases_levels.1": "Låga halter",
  "editor.phrases_levels.2": "Låga–måttliga halter",
  "editor.phrases_levels.3": "Måttliga halter",
  "editor.phrases_levels.4": "Måttliga–höga halter",
  "editor.phrases_levels.5": "Höga halter",
  "editor.phrases_levels.6": "Mycket höga halter",
  "editor.phrases_short": "Allergener, kort",
  "editor.phrases_short.alder": "Al",
  "editor.phrases_short.ash": "Ask",
  "editor.phrases_short.birch": "Björk",
  "editor.phrases_short.cypress": "Cyp.",
  "editor.phrases_short.elm": "Alm",
  "editor.phrases_short.grass": "Gräs",
  "editor.phrases_short.hazel": "Hass",
  "editor.phrases_short.lime": "Lind",
  "editor.phrases_short.mold_spores": "Mögel",
  "editor.phrases_short.mugwort": "Gråbo",
  "editor.phrases_short.nettle_and_pellitory": "Nässla",
  "editor.phrases_short.oak": "Ek",
  "editor.phrases_short.olive": "Oliv",
  "editor.phrases_short.plane": "Platan",
  "editor.phrases_short.ragweed": "Ambro",
  "editor.phrases_short.rye": "Råg",
  "editor.phrases_short.willow": "Vide",
  "editor.phrases_translate_all": "Översätt allt",
  "editor.pollen_threshold": "Tröskelvärde:",
  "editor.preset_reset_all": "Återställ allt",
  "editor.region_id": "Region ID",
  "editor.show_empty_days": "Visa tomma dagar",
  "editor.show_text_allergen": "Visa text, allergen",
  "editor.show_value_numeric": "Visa värde, numeriskt",
  "editor.show_value_numeric_in_circle": "Visa numeriskt värde inuti cirklarna",
  "editor.show_value_text": "Visa värde, text",
  "editor.sort": "Sortering",
  "editor.title": "Titel (tom = auto)",
  "card.integration.peu": "Polleninformation EU"
}, dC = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: UC
}, Symbol.toStringTag, { value: "Module" })), VA = /* @__PURE__ */ Object.assign({ "./locales/de.json": EC, "./locales/en.json": fC, "./locales/sv.json": dC }), eA = {};
for (const e in VA) {
  const A = e.match(/\.\/locales\/([\w-]+)\.json$/);
  A && (eA[A[1]] = VA[e].default);
}
const yA = "en";
function XA(e, A) {
  return A.split(".").reduce((g, I) => g && typeof g == "object" ? g[I] : void 0, e);
}
function q(e, A) {
  var n;
  let g = A || ((n = e == null ? void 0 : e.locale) == null ? void 0 : n.language) || (e == null ? void 0 : e.language) || yA;
  if (eA[g])
    return g;
  const I = g.slice(0, 2).toLowerCase();
  return eA[I] ? I : yA;
}
const mC = Object.keys(eA);
function u(e, A) {
  const g = eA[A] || {};
  if (g[e] !== void 0)
    return g[e];
  const I = XA(g, e);
  if (I !== void 0)
    return I;
  const n = eA[yA] || {};
  if (n[e] !== void 0)
    return n[e];
  const C = XA(n, e);
  return C !== void 0 ? C : e;
}
function HA(e) {
  return e.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
}
function xC(e) {
  return e.toLowerCase().replace(/ä/g, "ae").replace(/ö/g, "oe").replace(/ü/g, "ue").replace(/ß/g, "ss").replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
}
const BC = "state_tomorrow", uC = "state_in_2_days", pC = "state_today_desc", cC = "state_tomorrow_desc", yC = "state_in_2_days_desc", Y = {
  integration: "dwd",
  region_id: "",
  allergens: [
    "erle",
    "ambrosia",
    "esche",
    "birke",
    "hasel",
    "gräser",
    "beifuss",
    "roggen"
  ],
  minimal: !1,
  show_text_allergen: !0,
  show_value_text: !0,
  show_value_numeric: !1,
  show_value_numeric_in_circle: !1,
  show_empty_days: !0,
  debug: !1,
  days_to_show: 2,
  days_relative: !0,
  days_abbreviated: !1,
  days_uppercase: !1,
  days_boldfaced: !1,
  pollen_threshold: 0.5,
  sort: "value_descending",
  allergens_abbreviated: !1,
  date_locale: void 0,
  title: void 0,
  phrases: {
    full: {},
    short: {},
    levels: [],
    days: {},
    no_information: ""
  }
};
async function HC(e, A) {
  const g = !!A.debug, I = (B) => B.charAt(0).toUpperCase() + B.slice(1), n = q(e, A.date_locale), C = A.date_locale || Y.date_locale, D = A.days_relative !== !1, t = !!A.days_abbreviated, h = !!A.days_uppercase, i = A.phrases || {}, a = i.full || {}, r = i.short || {}, Q = i.levels, l = Array.isArray(Q) && Q.length === 7 ? Q : Array.from({ length: 7 }, (B, f) => u(`card.levels.${f}`, n)), m = i.no_information || u("card.no_information", n), R = i.days || {}, T = A.days_to_show ?? Y.days_to_show, AA = A.pollen_threshold ?? Y.pollen_threshold;
  g && console.debug("DWD adapter: start fetchForecast", { config: A, lang: n });
  const M = (B) => {
    const f = Number(B);
    return isNaN(f) ? -1 : f;
  }, W = /* @__PURE__ */ new Date();
  W.setHours(0, 0, 0, 0);
  const Z = [];
  for (const B of A.allergens)
    try {
      const f = {}, v = xC(B);
      f.allergenReplaced = v;
      const x = $[v] || v, E = a[B];
      if (E)
        f.allergenCapitalized = E;
      else {
        const c = `card.allergen.${$[v] || v}`, k = u(c, n);
        f.allergenCapitalized = k !== c ? k : I(B);
      }
      if (A.allergens_abbreviated) {
        const p = r[B];
        f.allergenShort = p || u(`editor.phrases_short.${x}`, n) || f.allergenCapitalized;
      } else
        f.allergenShort = f.allergenCapitalized;
      let U = A.region_id ? `sensor.pollenflug_${v}_${A.region_id}` : null;
      if (!U || !e.states[U]) {
        const p = Object.keys(e.states).filter(
          (c) => c.startsWith(`sensor.pollenflug_${v}_`)
        );
        if (p.length === 1) U = p[0];
        else continue;
      }
      const s = e.states[U], H = M(s.state), G = M(s.attributes[BC]), L = M(s.attributes[uC]), F = [
        { date: W, level: H },
        { date: new Date(W.getTime() + 864e5), level: G },
        { date: new Date(W.getTime() + 2 * 864e5), level: L }
      ];
      for (; F.length < T; ) {
        const p = F.length;
        F.push({
          date: new Date(W.getTime() + p * 864e5),
          level: -1
        });
      }
      f.days = [], F.forEach((p, c) => {
        const k = Math.round((p.date - W) / 864e5);
        let S;
        D ? R[k] !== void 0 ? S = R[k] : k >= 0 && k <= 2 ? S = u(`card.days.${k}`, n) : S = p.date.toLocaleDateString(C, {
          day: "numeric",
          month: "short"
        }) : (S = p.date.toLocaleDateString(C, {
          weekday: t ? "short" : "long"
        }), S = S.charAt(0).toUpperCase() + S.slice(1)), h && (S = S.toUpperCase());
        const o = s.attributes[c === 0 ? pC : c === 1 ? cC : yC] || "", y = p.level * 2, P = Math.min(Math.max(Math.round(y), 0), 6), K = P < 0 ? m : l[P] || o;
        f[`day${c}`] = {
          name: f.allergenCapitalized,
          day: S,
          state: p.level,
          state_text: K
        }, f.days.push(f[`day${c}`]);
      }), (F.slice(0, T).some((p) => p.level >= AA) || AA === 0) && Z.push(f);
    } catch (f) {
      console.warn(`DWD adapter error for allergen ${B}:`, f);
    }
  return Z.sort(
    {
      value_ascending: (B, f) => B.day0.state - f.day0.state,
      value_descending: (B, f) => f.day0.state - B.day0.state,
      name_descending: (B, f) => f.allergenCapitalized.localeCompare(B.allergenCapitalized)
    }[A.sort] || ((B, f) => f.day0.state - B.day0.state)
  ), g && console.debug("DWD adapter complete sensors:", Z), Z;
}
const jC = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  fetchForecast: HC,
  stubConfigDWD: Y
}, Symbol.toStringTag, { value: "Module" })), b = {
  integration: "peu",
  location: "",
  allergens: [
    "alder",
    "ash",
    "beech",
    "birch",
    "cypress",
    "elm",
    "grass",
    "hazel",
    "lime",
    "mold_spores",
    "mugwort",
    "nettle_and_pellitory",
    "oak",
    "olive",
    "plane",
    "ragweed",
    "rye",
    "willow"
  ],
  minimal: !1,
  show_text_allergen: !0,
  show_value_text: !0,
  show_value_numeric: !1,
  show_value_numeric_in_circle: !1,
  show_empty_days: !0,
  debug: !1,
  days_to_show: 4,
  days_relative: !0,
  days_abbreviated: !1,
  days_uppercase: !1,
  days_boldfaced: !1,
  pollen_threshold: 1,
  sort: "value_descending",
  allergens_abbreviated: !1,
  date_locale: void 0,
  title: void 0,
  phrases: { full: {}, short: {}, levels: [], days: {}, no_information: "" }
};
async function RC(e, A) {
  var x, E;
  const g = !!A.debug, I = q(e, A.date_locale), n = A.date_locale || ((x = e.locale) == null ? void 0 : x.language) || e.language || `${I}-${I.toUpperCase()}`, C = A.days_relative !== !1, D = !!A.days_abbreviated, t = !!A.days_uppercase, h = {
    full: {},
    short: {},
    levels: [],
    days: {},
    no_information: "",
    ...A.phrases || {}
  }, i = h.full, a = h.short, r = h.levels, Q = 5, l = Array.isArray(r) && r.length === Q ? r : Array.from(
    { length: Q },
    (U, s) => u(`card.levels.${s}`, I)
  ), m = h.no_information || u("card.no_information", I), R = h.days, T = A.integration === "dwd" ? 3 : A.integration === "peu" ? 4 : 6, AA = (U) => {
    const s = Number(U);
    return isNaN(s) || s < 0 ? -1 : s > T ? T : s;
  }, M = /* @__PURE__ */ new Date();
  M.setHours(0, 0, 0, 0);
  const W = A.days_to_show ?? b.days_to_show, Z = A.pollen_threshold ?? b.pollen_threshold, B = Object.keys(e.states).filter(
    (U) => U.startsWith("sensor.polleninformation_")
  );
  let f = A.location;
  !f && B.length && (f = B[0].slice(25).split("_")[0]);
  const v = [];
  for (const U of A.allergens)
    try {
      const s = {};
      s.days = [];
      const H = U;
      if (s.allergenReplaced = H, i[H])
        s.allergenCapitalized = i[H];
      else {
        const o = $[H] || H, y = u(`card.allergen.${o}`, I);
        s.allergenCapitalized = y !== `card.allergen.${o}` ? y : H.charAt(0).toUpperCase() + H.slice(1);
      }
      if (A.allergens_abbreviated) {
        const o = a[H];
        s.allergenShort = o || u(`editor.phrases_short.${H}`, I) || s.allergenCapitalized;
      } else
        s.allergenShort = s.allergenCapitalized;
      let G = f ? `sensor.polleninformation_${f}_${H}` : null;
      if (!G || !e.states[G]) {
        const o = B.filter((y) => y.endsWith(`_${H}`));
        if (o.length === 1) G = o[0];
        else continue;
      }
      const L = e.states[G];
      if (!((E = L == null ? void 0 : L.attributes) != null && E.forecast)) throw "Missing forecast";
      const F = L.attributes.forecast, QA = Array.isArray(F) ? F.reduce((o, y) => {
        const P = y.time || y.datetime;
        return o[P] = y, o;
      }, {}) : {}, c = Object.keys(QA).sort(
        (o, y) => new Date(o) - new Date(y)
      ).filter((o) => new Date(o) >= M);
      let k = [];
      if (c.length >= W)
        k = c.slice(0, W);
      else {
        k = c.slice();
        let o = c.length > 0 ? new Date(c[c.length - 1]) : M;
        for (; k.length < W; ) {
          o = new Date(o.getTime() + 864e5);
          const y = o.getFullYear(), P = String(o.getMonth() + 1).padStart(2, "0"), K = String(o.getDate()).padStart(2, "0");
          k.push(`${y}-${P}-${K}T00:00:00`);
        }
      }
      k.forEach((o, y) => {
        const P = QA[o] || {}, K = AA(P.level), V = new Date(o), J = Math.round((V - M) / 864e5);
        let O;
        C ? R[J] != null ? O = R[J] : J >= 0 && J <= 2 ? O = u(`card.days.${J}`, I) : O = V.toLocaleDateString(n, {
          day: "numeric",
          month: "short"
        }) : (O = V.toLocaleDateString(n, {
          weekday: D ? "short" : "long"
        }), O = O.charAt(0).toUpperCase() + O.slice(1)), t && (O = O.toUpperCase());
        const JA = {
          name: s.allergenCapitalized,
          day: O,
          state: K,
          state_text: K < 0 ? m : l[K]
        };
        s[`day${y}`] = JA, s.days.push(JA);
      }), (s.days.some((o) => o.state >= Z) || Z === 0) && v.push(s);
    } catch (s) {
      g && console.warn(`Fel vid allergen ${U}:`, s);
    }
  return v.sort(
    {
      value_ascending: (U, s) => U.day0.state - s.day0.state,
      value_descending: (U, s) => s.day0.state - U.day0.state,
      name_ascending: (U, s) => U.allergenCapitalized.localeCompare(s.allergenCapitalized),
      name_descending: (U, s) => s.allergenCapitalized.localeCompare(U.allergenCapitalized)
    }[A.sort] || ((U, s) => s.day0.state - U.day0.state)
  ), g && console.debug("PEU.fetchForecast — done", v), v;
}
const wC = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  fetchForecast: RC,
  stubConfigPEU: b
}, Symbol.toStringTag, { value: "Module" })), JC = {
  pp: qA,
  dwd: jC,
  peu: wC
}, Cg = {
  11: "Schleswig-Holstein und Hamburg",
  12: "Schleswig-Holstein und Hamburg",
  20: "Mecklenburg-Vorpommern",
  31: "Niedersachsen und Bremen",
  32: "Niedersachsen und Bremen",
  41: "Nordrhein-Westfalen",
  42: "Nordrhein-Westfalen",
  43: "Nordrhein-Westfalen",
  50: "Brandenburg und Berlin",
  61: "Sachsen-Anhalt",
  62: "Sachsen-Anhalt",
  71: "Thüringen",
  72: "Thüringen",
  81: "Sachsen",
  82: "Sachsen",
  91: "Hessen",
  92: "Hessen",
  101: "Rheinland-Pfalz und Saarland",
  102: "Rheinland-Pfalz und Saarland",
  103: "Rheinland-Pfalz und Saarland",
  111: "Baden-Württemberg",
  112: "Baden-Württemberg",
  113: "Baden-Württemberg",
  121: "Bayern",
  122: "Bayern",
  123: "Bayern",
  124: "Bayern"
}, $ = {
  // Svenska
  al: "alder",
  alm: "elm",
  bok: "beech",
  bjork: "birch",
  ek: "oak",
  grabo: "mugwort",
  gras: "grass",
  hassel: "hazel",
  malortsambrosia: "ragweed",
  salg_och_viden: "willow",
  // Tyska (DWD), normaliserade via replaceAAO
  erle: "alder",
  ambrosia: "ragweed",
  esche: "ash",
  birke: "birch",
  buche: "beech",
  hasel: "hazel",
  graser: "grass",
  // från 'gräser'
  graeser: "grass",
  // från 'gräser'
  beifuss: "mugwort",
  // från 'beifuss'
  roggen: "rye",
  // Engelska (PEU)
  olive: "olive",
  plane: "plane",
  cypress: "cypress",
  lime: "lime",
  mold_spores: "mold_spores",
  nettle_and_pellitory: "nettle_and_pellitory"
}, jA = [
  "Borlänge",
  "Bräkne-Hoby",
  "Eskilstuna",
  "Forshaga",
  "Gävle",
  "Göteborg",
  "Hässleholm",
  "Jönköping",
  "Kristianstad",
  "Ljusdal",
  "Malmö",
  "Norrköping",
  "Nässjö",
  "Piteå",
  "Skövde",
  "Stockholm",
  "Storuman",
  "Sundsvall",
  "Umeå",
  "Visby",
  "Västervik",
  "Östersund"
], w = {
  integration: "pp",
  city: "",
  allergens: [
    "Al",
    "Alm",
    "Bok",
    "Björk",
    "Ek",
    "Malörtsambrosia",
    "Gråbo",
    "Gräs",
    "Hassel",
    "Sälg och viden"
  ],
  minimal: !1,
  show_text_allergen: !0,
  show_value_text: !0,
  show_value_numeric: !1,
  show_value_numeric_in_circle: !1,
  show_empty_days: !0,
  debug: !1,
  days_to_show: 4,
  days_relative: !0,
  days_abbreviated: !1,
  days_uppercase: !1,
  days_boldfaced: !1,
  pollen_threshold: 1,
  sort: "value_descending",
  allergens_abbreviated: !1,
  date_locale: void 0,
  title: void 0,
  phrases: { full: {}, short: {}, levels: [], days: {}, no_information: "" }
};
async function vC(e, A) {
  var f, v;
  const g = [], I = !!A.debug, n = (x) => x.charAt(0).toUpperCase() + x.slice(1), C = (x) => {
    const [E] = x.split("T"), [U, s, H] = E.split("-").map(Number);
    return new Date(U, s - 1, H);
  }, D = /* @__PURE__ */ new Date();
  D.setHours(0, 0, 0, 0);
  const t = q(e, A.date_locale), h = A.date_locale || ((f = e.locale) == null ? void 0 : f.language) || e.language || `${t}-${t.toUpperCase()}`, i = A.days_relative !== !1, a = !!A.days_abbreviated, r = !!A.days_uppercase, Q = {
    full: {},
    short: {},
    levels: [],
    days: {},
    no_information: "",
    ...A.phrases || {}
  }, l = Q.full, m = Q.short, R = Q.levels, T = Array.isArray(R) && R.length === 7 ? R : Array.from({ length: 7 }, (x, E) => u(`card.levels.${E}`, t)), AA = Q.no_information || u("card.no_information", t), M = Q.days;
  I && console.debug("PP.fetchForecast — start", { city: A.city, lang: t });
  const W = (x) => {
    const E = Number(x);
    return isNaN(E) || E < 0 ? -1 : E > 6 ? 6 : E;
  }, Z = A.days_to_show ?? w.days_to_show, B = A.pollen_threshold ?? w.pollen_threshold;
  for (const x of A.allergens)
    try {
      const E = {};
      E.days = [];
      const U = HA(x);
      if (E.allergenReplaced = U, l[x])
        E.allergenCapitalized = l[x];
      else {
        const S = $[U] || U, o = u(`card.allergen.${S}`, t);
        E.allergenCapitalized = o !== `card.allergen.${S}` ? o : n(x);
      }
      if (A.allergens_abbreviated) {
        const S = $[U] || U, o = m[x];
        E.allergenShort = o || u(`editor.phrases_short.${S}`, t) || E.allergenCapitalized;
      } else
        E.allergenShort = E.allergenCapitalized;
      const s = HA(A.city);
      let H = `sensor.pollen_${s}_${U}`;
      if (!e.states[H]) {
        const S = Object.keys(e.states).filter(
          (o) => o.startsWith(`sensor.pollen_${s}_`) && o.includes(U)
        );
        if (S.length === 1) H = S[0];
        else continue;
      }
      const G = e.states[H];
      if (!((v = G == null ? void 0 : G.attributes) != null && v.forecast)) throw "Missing forecast";
      const L = G.attributes.forecast, F = Array.isArray(L) ? L.reduce((S, o) => {
        const y = o.time || o.datetime;
        return S[y] = o, S;
      }, {}) : L, p = Object.keys(F).sort(
        (S, o) => C(S) - C(o)
      ).filter((S) => C(S) >= D);
      let c = [];
      if (p.length >= Z)
        c = p.slice(0, Z);
      else {
        c = p.slice();
        let S = p.length > 0 ? C(p[p.length - 1]) : D;
        for (; c.length < Z; ) {
          S = new Date(S.getTime() + 864e5);
          const o = S.getFullYear(), y = String(S.getMonth() + 1).padStart(2, "0"), P = String(S.getDate()).padStart(2, "0");
          c.push(`${o}-${y}-${P}T00:00:00`);
        }
      }
      c.forEach((S, o) => {
        const y = F[S] || {}, P = W(y.level), K = C(S), V = Math.round((K - D) / 864e5);
        let J;
        i ? M[V] != null ? J = M[V] : V >= 0 && V <= 2 ? J = u(`card.days.${V}`, t) : J = K.toLocaleDateString(h, {
          day: "numeric",
          month: "short"
        }) : (J = K.toLocaleDateString(h, {
          weekday: a ? "short" : "long"
        }), J = J.charAt(0).toUpperCase() + J.slice(1)), r && (J = J.toUpperCase());
        const O = {
          name: E.allergenCapitalized,
          day: J,
          state: P,
          state_text: P < 0 ? AA : T[P]
        };
        E[`day${o}`] = O, E.days.push(O);
      }), (E.days.some((S) => S.state >= B) || B === 0) && g.push(E);
    } catch (E) {
      console.warn(`Fel vid allergen ${x}:`, E);
    }
  return g.sort(
    {
      value_ascending: (x, E) => x.day0.state - E.day0.state,
      value_descending: (x, E) => E.day0.state - x.day0.state,
      name_ascending: (x, E) => x.allergenCapitalized.localeCompare(E.allergenCapitalized),
      name_descending: (x, E) => E.allergenCapitalized.localeCompare(x.allergenCapitalized)
    }[A.sort] || ((x, E) => E.day0.state - x.day0.state)
  ), I && console.debug("PP.fetchForecast — done", g), g;
}
const kC = JC;
class ng extends IA {
  get debug() {
    return !0;
  }
  get _lang() {
    var A;
    return q(this._hass, (A = this.config) == null ? void 0 : A.date_locale);
  }
  _t(A) {
    return u(A, this._lang);
  }
  _getImageSrc(A, g) {
    const I = Number(g);
    let n = I, C = -1, D = 6;
    this.config.integration === "dwd" ? (n = I * 2, D = 6) : this.config.integration === "peu" && (n = I, D = 4);
    let t = Math.round(n);
    (isNaN(t) || t < C) && (t = C), t > D && (t = D);
    const h = $[A] || A;
    return TA[`${h}_${t}_png`] || TA[`${t}_png`];
  }
  constructor() {
    super(), this.days_to_show = 4, this.displayCols = [], this.header = "", this._initDone = !1, this._userConfig = {}, this.sensors = [], this.tapAction = null;
  }
  static async getConfigElement() {
    return await customElements.whenDefined("pollenprognos-card-editor"), document.createElement("pollenprognos-card-editor");
  }
  static getStubConfig() {
    return w;
  }
  setConfig(A) {
    if (this._userConfig = { ...A }, this.tapAction = A.tap_action || null, this._integrationExplicit = A.hasOwnProperty("integration") && A.integration !== w.integration, !this._integrationExplicit && this._hass) {
      const g = Object.keys(this._hass.states), I = g.some(
        (C) => C.startsWith("sensor.pollen_") && !C.startsWith("sensor.pollenflug_")
      ), n = g.some((C) => C.startsWith("sensor.pollenflug_"));
      !I && n && (this._userConfig.integration = "dwd", this.debug && console.debug("[PollenPrognos] fallback to DWD in setConfig"));
    }
    this._initDone = !1, this._hass && (this.hass = this._hass);
  }
  set hass(A) {
    var a, r, Q;
    this._hass = A;
    const g = !!this._integrationExplicit, I = Object.keys(A.states).filter(
      (l) => l.startsWith("sensor.pollen_") && !l.startsWith("sensor.pollenflug_")
    ), n = Object.keys(A.states).filter(
      (l) => l.startsWith("sensor.pollenflug_")
    ), C = Object.keys(A.states).filter(
      (l) => l.startsWith("sensor.polleninformation_")
    );
    let D = this._userConfig.integration;
    g || (I.length ? D = "pp" : C.length ? D = "peu" : n.length && (D = "dwd", this._userConfig = {})), g || (D === "pp" && !I.length && C.length ? (D = "peu", this._userConfig = {}) : (D === "pp" || D === "peu") && !I.length && !C.length && n.length && (D = "dwd", this._userConfig = {}));
    let t;
    D === "dwd" ? t = Y : D === "peu" ? t = b : t = w;
    const h = { ...t, ...this._userConfig, integration: D };
    if (!this._userConfig.hasOwnProperty("date_locale")) {
      const l = q(A, null), m = ((r = (a = this._hass) == null ? void 0 : a.locale) == null ? void 0 : r.language) || ((Q = this._hass) == null ? void 0 : Q.language) || `${l}-${l.toUpperCase()}`;
      h.date_locale = m, this.debug && console.debug(
        "[PollenPrognos] auto-filling date_locale:",
        h.date_locale
      );
    }
    if (D === "dwd" && !h.region_id && n.length ? h.region_id = Array.from(
      new Set(n.map((l) => l.split("_").pop()))
    ).sort((l, m) => Number(l) - Number(m))[0] : D === "pp" && !h.city && I.length && (h.city = I[0].slice(14).replace(/_[^_]+$/, "")), this.config = h, this.tapAction = h.tap_action || this.tapAction || null, typeof h.title == "string")
      this.header = h.title;
    else if (h.title === !1)
      this.header = "";
    else {
      const l = D === "dwd" ? Cg[h.region_id] || h.region_id : jA.find(
        (m) => m.toLowerCase().replace(/[åä]/g, "a").replace(/ö/g, "o").replace(/[-\s]/g, "_") === h.city
      ) || h.city;
      this.header = `${this._t("card.header_prefix")} ${l}`;
    }
    (kC[h.integration] || qA).fetchForecast(A, h).then((l) => {
      var m;
      this.sensors = l, this.days_to_show = h.days_to_show, this.displayCols = Array.from(
        {
          length: h.show_empty_days ? h.days_to_show : ((m = l[0]) == null ? void 0 : m.days.length) || 0
        },
        (R, T) => T
      ), this.requestUpdate();
    }).catch((l) => console.error("Error fetching pollen forecast:", l));
  }
  _renderMinimalHtml() {
    return d`
      <ha-card>
        ${this.header ? d`<h1 class="header">${this.header}</h1>` : ""}
        <div class="flex-container">
          ${this.sensors.map((A) => {
      const g = A.day0.state_text || "", I = A.day0.state;
      let n = "";
      return this.config.show_text_allergen && (n += this.config.allergens_abbreviated ? A.allergenShort : A.allergenCapitalized), this.config.show_value_text && this.config.show_value_numeric ? (n && (n += ": "), n += `${g} (${I})`) : this.config.show_value_text ? (n && (n += ": "), n += g) : this.config.show_value_numeric && (n && (n += " "), n += `(${I})`), d`
              <div class="sensor">
                <img
                  class="box"
                  src="${this._getImageSrc(
        A.allergenReplaced,
        A.day0.state
      )}"
                />
                ${n ? d`<span class="short-text">${n}</span>` : ""}
              </div>
            `;
    })}
        </div>
      </ha-card>
    `;
  }
  _renderNormalHtml() {
    const A = !!this.config.days_boldfaced, g = this.displayCols;
    return d`
      <ha-card>
        ${this.header ? d`<h1 class="header">${this.header}</h1>` : ""}
        <table class="forecast">
          <thead>
            <tr>
              <th></th>
              ${g.map(
      (I) => {
        var n;
        return d`
                  <th style="font-weight: ${A ? "bold" : "normal"}">
                    ${((n = this.sensors[0].days[I]) == null ? void 0 : n.day) || ""}
                  </th>
                `;
      }
    )}
            </tr>
          </thead>

          ${this.sensors.map(
      (I) => {
        var n;
        return d`
              <!-- Rad 1: bara ikoner -->
              <tr class="allergen-icon-row" valign="top">
                <td>
                  <img
                    class="allergen"
                    src="${this._getImageSrc(
          I.allergenReplaced,
          (n = I.days[0]) == null ? void 0 : n.state
        )}"
                  />
                </td>
                ${g.map(
          (C) => {
            var D, t;
            return d`
                    <td>
                      <div class="icon-wrapper">
                        <img
                          src="${this._getImageSrc("", (D = I.days[C]) == null ? void 0 : D.state)}"
                        />
                        ${this.config.show_value_numeric_in_circle ? d`<span class="circle-overlay">
                              ${((t = I.days[C]) == null ? void 0 : t.state) ?? ""}
                            </span>` : ""}
                      </div>
                    </td>
                  `;
          }
        )}
              </tr>

              <!-- Rad 2: allergennamn + text/nummer under dagarna -->
              ${this.config.show_text_allergen || this.config.show_value_text || this.config.show_value_numeric ? d`
                    <tr class="allergen-text-row" valign="top">
                      <td>
                        ${this.config.show_text_allergen ? this.config.allergens_abbreviated ? I.allergenShort : I.allergenCapitalized : ""}
                      </td>
                      ${g.map((C) => {
          var i, a;
          const D = ((i = I.days[C]) == null ? void 0 : i.state_text) || "", t = (a = I.days[C]) == null ? void 0 : a.state;
          let h = "";
          return this.config.show_value_text && this.config.show_value_numeric ? h = `${D} (${t})` : this.config.show_value_text ? h = D : this.config.show_value_numeric && (h = String(t)), d`<td>${h}</td>`;
        })}
                    </tr>
                  ` : ""}
            `;
      }
    )}
        </table>
      </ha-card>
    `;
  }
  render() {
    if (!this.sensors.length) {
      const g = `card.integration.${this.config.integration}`, I = this._t(g);
      return d`
        <ha-card
          @click="${this._handleTapAction}"
          style="cursor: ${this.tapAction ? "pointer" : "auto"}"
        >
          <div class="card-error">${this._t("card.error")} (${I})</div>
        </ha-card>
      `;
    }
    const A = this.config.minimal ? this._renderMinimalHtml() : this._renderNormalHtml();
    return d`
      <ha-card
        @click="${this._handleTapAction}"
        style="cursor: ${this.tapAction ? "pointer" : "auto"}"
      >
        ${A}
      </ha-card>
    `;
  }
  getCardSize() {
    return this.sensors.length + 1;
  }
  _handleTapAction(A) {
    var n, C;
    if (!this.tapAction || !this._hass) return;
    (n = A.preventDefault) == null || n.call(A), (C = A.stopPropagation) == null || C.call(A);
    const g = this.tapAction.type || "more-info";
    let I = this.tapAction.entity || "camera.pollen";
    switch (g) {
      case "more-info":
        this._fire("hass-more-info", { entityId: I });
        break;
      case "navigate":
        this.tapAction.navigation_path && window.history.pushState(null, "", this.tapAction.navigation_path);
        break;
      case "call-service":
        if (this.tapAction.service && typeof this.tapAction.service == "string") {
          const [D, t] = this.tapAction.service.split(".");
          this._hass.callService(
            D,
            t,
            this.tapAction.service_data || {}
          );
        }
        break;
    }
  }
  _fire(A, g, I) {
    const n = new Event(A, {
      bubbles: !0,
      cancelable: !1,
      composed: !0,
      ...I
    });
    return n.detail = g, this.dispatchEvent(n), n;
  }
  static get styles() {
    return NA`
      .header {
        margin: 0;
        padding: 4px 16px 12px;
        @apply --paper-font-headline;
        line-height: 40px;
        color: var(--primary-text-color);
      }
      .forecast {
        width: 100%;
        padding: 7px;
        border-collapse: separate;
        border-spacing: 0 4px;
      }
      .icon-wrapper {
        position: relative;
        display: inline-block;
      }
      .icon-wrapper .circle-overlay {
        position: absolute;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%);
        font-size: 0.75rem;
        font-weight: bold;
        color: var(--primary-text-color);
        pointer-events: none;
      }
      td {
        padding: 1px;
        text-align: center;
        width: 100px;
        font-size: smaller;
      }
      img.allergen {
        width: 40px;
        height: 40px;
      }
      img {
        width: 50px;
        height: 50px;
      }
      .flex-container {
        display: flex;
        flex-wrap: wrap;
        justify-content: space-evenly;
        align-items: center;
        padding: 16px;
      }
      .sensor {
        flex: 1;
        min-width: 20%;
        text-align: center;
      }
      .short-text {
        display: block;
      }
      .card-error {
        padding: 16px;
        color: var(--error-text-color, #b71c1c);
        font-weight: 500;
        line-height: 1.4;
      }
      .value-text {
        font-size: smaller;
        margin-top: 4px;
        display: block;
        text-align: center;
      }
    `;
  }
}
vA(ng, "properties", {
  hass: { state: !0 },
  config: {},
  sensors: { state: !0 },
  days_to_show: { state: !0 },
  displayCols: { state: !0 },
  header: { state: !0 },
  tapAction: {}
});
customElements.define("pollenprognos-card", ng);
const tA = (e, A) => {
  const g = { ...e };
  for (const I of Object.keys(A)) {
    const n = A[I];
    n !== null && typeof n == "object" && !Array.isArray(n) && typeof e[I] == "object" && e[I] !== null ? g[I] = tA(e[I], n) : g[I] = n;
  }
  return g;
}, PC = (e) => e === "dwd" ? Y : e === "peu" ? b : w;
class OC extends IA {
  get debug() {
    return !0;
  }
  _resetAll() {
    this.debug && console.debug("[Editor] resetAll"), this._userConfig = {}, this.setConfig({ integration: this._config.integration });
  }
  _resetPhrases(A) {
    this.debug && console.debug("[Editor] resetPhrases – lang:", A), this._updateConfig("date_locale", A);
    const g = this._config.integration === "dwd" ? Y.allergens : this._config.integration === "peu" ? b.allergens : w.allergens, I = {}, n = {};
    g.forEach((i) => {
      const a = HA(i), r = $[a] || a;
      I[i] = u(`editor.phrases_full.${r}`, A), n[i] = u(`editor.phrases_short.${r}`, A);
    });
    const C = this._config.integration === "dwd" ? 4 : this._config.integration === "peu" ? 5 : 7, D = Array.from(
      { length: C },
      (i, a) => u(`editor.phrases_levels.${a}`, A)
    ), t = {
      0: u("editor.phrases_days.0", A),
      1: u("editor.phrases_days.1", A),
      2: u("editor.phrases_days.2", A)
    }, h = u("editor.no_information", A);
    this._updateConfig("phrases", {
      full: I,
      short: n,
      levels: D,
      days: t,
      no_information: h
    });
  }
  static get properties() {
    return {
      _config: { type: Object },
      hass: { type: Object },
      installedCities: { type: Array },
      installedRegionIds: { type: Array },
      _initDone: { type: Boolean },
      _selectedPhraseLang: { state: !0 },
      _tapType: { type: String },
      _tapEntity: { type: String },
      _tapNavigation: { type: String },
      _tapService: { type: String },
      _tapServiceData: { type: String }
    };
  }
  get _lang() {
    return q(this._hass, this._config.date_locale);
  }
  _t(A) {
    return u(`editor.${A}`, this._lang);
  }
  constructor() {
    var A;
    super(), this._userConfig = {}, this._integrationExplicit = !1, this._thresholdExplicit = !1, this._config = tA(w, {}), this.installedCities = [], this.installedPeuLocations = [], this._prevIntegration = void 0, this.installedRegionIds = [], this._initDone = !1, this._selectedPhraseLang = q(
      this._hass,
      (A = this._config) == null ? void 0 : A.date_locale
    ), this._allergensExplicit = !1, this._origAllergensSet = !1, this._userAllergens = null, this._tapType = "none", this._tapEntity = "", this._tapNavigation = "", this._tapService = "", this._tapServiceData = "";
  }
  setConfig(A) {
    var a, r;
    this.debug && console.debug("[Editor] ▶️ setConfig INCOMING:", A);
    const g = A.integration === "dwd" ? Y.allergens.length : A.integration === "peu" ? b.allergens.length : w.allergens.length;
    Array.isArray(A.allergens) && A.allergens.length < g && (this._userConfig.allergens = [...A.allergens], this._allergensExplicit = !0, this.debug && console.debug(
      "[Editor] saved user-chosen allergens:",
      this._userConfig.allergens
    ));
    const I = { ...A };
    Array.isArray(I.allergens) && I.allergens.length === g && (this.debug && console.debug(
      "[Editor] dropping incoming stub-allergens (length matches stub)"
    ), delete I.allergens), this.debug && console.debug(
      "[Editor][B] efter stub-drop, incoming.allergens:",
      I.allergens
    );
    const n = (I.integration === "dwd" ? Y : I.integration === "peu" ? b : w).pollen_threshold;
    I.hasOwnProperty("pollen_threshold") && !this._thresholdExplicit && I.pollen_threshold === n && (this.debug && console.debug(
      "[Editor] dropping incoming stub-threshold (matches stub):",
      n
    ), delete I.pollen_threshold);
    const C = A.integration;
    this._prevIntegration !== void 0 && C !== this._prevIntegration && (delete this._userConfig.allergens, this._allergensExplicit = !1, this.debug && console.debug("[Editor] integration changed → wipe allergens")), !this._integrationExplicit && I.integration === w.integration && (this.debug && console.debug("[Editor] dropped stub integration"), delete I.integration), !this._daysExplicit && I.days_to_show === w.days_to_show && (this.debug && console.debug("[Editor] dropped stub days_to_show"), delete I.days_to_show);
    const D = (I.integration === "dwd" ? Y : I.integration === "peu" ? b : w).date_locale;
    !this._localeExplicit && I.date_locale === D && (this.debug && console.debug("[Editor] dropped stub date_locale"), delete I.date_locale), this.debug && console.debug(
      "[Editor][C] före merge, userConfig:",
      this._userConfig,
      " incoming:",
      I
    ), this._userConfig = tA(this._userConfig, I), this._thresholdExplicit = this._userConfig.hasOwnProperty("pollen_threshold"), this.debug && console.debug(
      "[Editor][D] efter merge, this._userConfig:",
      this._userConfig
    ), this.debug && console.debug("[Editor]    _userConfig after merge:", this._userConfig), this._allergensExplicit = this._userConfig.hasOwnProperty("allergens"), this._integrationExplicit = this._userConfig.hasOwnProperty("integration"), this._daysExplicit = this._userConfig.hasOwnProperty("days_to_show"), this._localeExplicit = this._userConfig.hasOwnProperty("date_locale");
    let t = this._userConfig.integration !== void 0 ? this._userConfig.integration : this._config.integration;
    if (!this._integrationExplicit && this._hass) {
      const Q = Object.keys(this._hass.states);
      Q.some((l) => l.startsWith("sensor.pollen_")) ? t = "pp" : Q.some((l) => l.startsWith("sensor.polleninformation_")) ? t = "peu" : Q.some((l) => l.startsWith("sensor.pollenflug_")) && (t = "dwd"), this._userConfig.integration = t, this.debug && console.debug("[Editor] auto-detected integration:", t);
    }
    const h = PC(t);
    h.allergens, this.debug && console.debug("[Editor][E] baseStub.allergens:", h.allergens), this.debug && console.debug(
      "[Editor][E] this._userConfig.allergens:",
      this._userConfig.allergens
    );
    let i = tA(h, this._userConfig);
    if (this._userConfig.hasOwnProperty("pollen_threshold") || (i.pollen_threshold = h.pollen_threshold, this.debug && console.debug(
      "[Editor] reset pollen_threshold to stub:",
      h.pollen_threshold
    )), this.debug && console.debug(
      "[Editor] baseStub.allergens:",
      h.allergens,
      "userConfig.allergens:",
      this._userConfig.allergens
    ), i.allergens = Array.isArray(this._userConfig.allergens) && this._userConfig.allergens.length ? this._userConfig.allergens : h.allergens, this._config = i, this._prevIntegration = t, this.debug && console.debug(
      "[Editor][F] slutgiltigt this._config.allergens:",
      this._config.allergens
    ), this._config.integration = t, this._config.type = "custom:pollenprognos-card", this._daysExplicit || (this._config.days_to_show = h.days_to_show, this.debug && console.debug(
      "[Editor] reset days_to_show to stub:",
      h.days_to_show
    )), !this._localeExplicit) {
      const Q = q(this._hass, null), l = ((r = (a = this._hass) == null ? void 0 : a.locale) == null ? void 0 : r.language) || `${Q}-${Q.toUpperCase()}`;
      this._config.date_locale = l, this.debug && console.debug(
        "[Editor] autofilled date_locale:",
        l,
        "(HA language was:",
        Q,
        ")"
      );
    }
    if (this._initDone = !1, this._hass) {
      const Q = Object.keys(this._hass.states);
      this.installedRegionIds = Array.from(
        new Set(
          Q.filter((m) => m.startsWith("sensor.pollenflug_")).map((m) => m.split("_").pop())
        )
      ).sort((m, R) => Number(m) - Number(R));
      const l = new Set(
        Q.filter(
          (m) => m.startsWith("sensor.pollen_") && !m.startsWith("sensor.pollenflug_")
        ).map(
          (m) => m.slice(14).replace(/_[^_]+$/, "")
        )
      );
      this.installedCities = jA.filter(
        (m) => l.has(
          m.toLowerCase().replace(/[åä]/g, "a").replace(/ö/g, "o").replace(/[-\s]/g, "_")
        )
      ).sort();
    }
    this._integrationExplicit || (t === "dwd" && !this._userConfig.region_id && this.installedRegionIds.length && (this._config.region_id = this.installedRegionIds[0]), t === "pp" && !this._userConfig.city && this.installedCities.length && (this._config.city = this.installedCities[0])), this.debug && console.debug("[Editor] färdig _config innan dispatch:", this._config), this._config.tap_action ? (this._tapType = this._config.tap_action.type || "more-info", this._tapEntity = this._config.tap_action.entity || "", this._tapNavigation = this._config.tap_action.navigation_path || "", this._tapService = this._config.tap_action.service || "", this._tapServiceData = JSON.stringify(
      this._config.tap_action.service_data || {},
      null,
      2
    )) : (this._tapType = "none", this._tapEntity = "", this._tapNavigation = "", this._tapService = "", this._tapServiceData = ""), this.dispatchEvent(
      new CustomEvent("config-changed", {
        detail: { config: this._config },
        bubbles: !0,
        composed: !0
      })
    ), this.requestUpdate(), this._prevIntegration = C, this._initDone = !0;
  }
  set hass(A) {
    this._hass = A;
    const g = this._integrationExplicit, I = Object.keys(A.states).filter(
      (i) => i.startsWith("sensor.pollen_") && !i.startsWith("sensor.pollenflug_")
    ), n = Object.keys(A.states).filter(
      (i) => i.startsWith("sensor.pollenflug_")
    ), C = Object.keys(A.states).filter(
      (i) => i.startsWith("sensor.polleninformation_")
    );
    let D = this._userConfig.integration;
    g || (I.length ? D = "pp" : C.length ? D = "peu" : n.length && (D = "dwd"), this._userConfig.integration = D);
    const t = D === "dwd" ? Y : D === "peu" ? b : w;
    this._userConfig.hasOwnProperty("pollen_threshold") || (merged.pollen_threshold = t.pollen_threshold, this.debug && console.debug(
      "[Editor][hass] reset pollen_threshold to stub:",
      t.pollen_threshold
    )), this._config = merged, this.installedRegionIds = Array.from(
      new Set(n.map((i) => i.split("_").pop()))
    ).sort((i, a) => Number(i) - Number(a));
    const h = Array.from(
      new Set(
        I.map(
          (i) => i.slice(14).replace(/_[^_]+$/, "")
        )
      )
    );
    this.installedCities = jA.filter(
      (i) => h.includes(
        i.toLowerCase().replace(/[åä]/g, "a").replace(/ö/g, "o").replace(/[-\s]/g, "_")
      )
    ).sort((i, a) => i.localeCompare(a)), this.installedPeuLocations = Array.from(
      new Map(
        Object.values(A.states).filter((i) => i.entity_id.startsWith("sensor.polleninformation_")).map((i) => {
          var a, r;
          return [
            i.attributes.location_slug || i.entity_id.split("_")[1],
            i.attributes.location_title || ((r = (a = i.attributes.friendly_name) == null ? void 0 : a.match(/\((.*?)\)/)) == null ? void 0 : r[1]) || i.entity_id.split("_")[1]
          ];
        })
      )
    ), this._initDone || (D === "dwd" && !this._userConfig.region_id && this.installedRegionIds.length && (this._config.region_id = this.installedRegionIds[0]), D === "pp" && !this._userConfig.city && this.installedCities.length && (this._config.city = this.installedCities[0])), this._initDone = !0, this.dispatchEvent(
      new CustomEvent("config-changed", {
        detail: { config: this._config },
        bubbles: !0,
        composed: !0
      })
    ), this.requestUpdate();
  }
  _onAllergenToggle(A, g) {
    const I = new Set(this._config.allergens);
    g ? I.add(A) : I.delete(A), this._updateConfig("allergens", [...I]);
  }
  _updateConfig(A, g) {
    this.debug && console.debug("[Editor] _updateConfig – prop:", A, "value:", g);
    const I = { ...this._userConfig };
    let n;
    if (A === "integration") {
      const C = g, D = this._config.integration;
      C !== D && (delete I[C === "dwd" ? "city" : "region_id"], delete I.allergens, delete I.pollen_threshold, this._allergensExplicit = !1), n = tA(C === "dwd" ? Y : C === "peu" ? b : w, I), n.integration = C;
    } else A === "tap_action" ? n = { ...this._config, tap_action: g } : n = { ...this._config, [A]: g };
    n.type = this._config.type, this._config = n, this.debug && console.debug("[Editor] updated _config:", this._config), this.dispatchEvent(
      new CustomEvent("config-changed", {
        detail: { config: this._config },
        bubbles: !0,
        composed: !0
      })
    );
  }
  render() {
    const A = {
      phrases: {
        full: {},
        short: {},
        levels: [],
        days: {},
        no_information: ""
      },
      ...this._config
    }, g = A.integration === "dwd" ? Y.allergens : A.integration === "peu" ? b.allergens : w.allergens, I = A.integration === "dwd" ? 4 : A.integration === "peu" ? 5 : 7, n = A.integration === "dwd" ? { min: 0, max: 3, step: 0.5 } : A.integration === "peu" ? { min: 0, max: 4, step: 1 } : { min: 0, max: 6, step: 1 };
    return d`
      <div class="card-config">
        <!-- Återställ-knapp -->
        <div class="preset-buttons">
          <mwc-button @click=${() => this._resetAll()}>
            ${this._t("preset_reset_all")}
          </mwc-button>
        </div>

        <!-- Integration -->
        <ha-formfield label="${this._t("integration")}">
          <ha-select
            .value=${A.integration}
            @selected=${(C) => this._updateConfig("integration", C.target.value)}
            @closed=${(C) => C.stopPropagation()}
          >
            <mwc-list-item value="pp"
              >${this._t("integration.pp")}</mwc-list-item
            >
            <mwc-list-item value="peu"
              >${this._t("integration.peu")}</mwc-list-item
            >
            <mwc-list-item value="dwd"
              >${this._t("integration.dwd")}</mwc-list-item
            >
          </ha-select>
        </ha-formfield>

        <!-- Stad (PP) eller Region (DWD) -->
        ${A.integration === "pp" ? d`
              <ha-formfield label="${this._t("city")}">
                <ha-select
                  .value=${A.city || ""}
                  @selected=${(C) => this._updateConfig("city", C.target.value)}
                  @closed=${(C) => C.stopPropagation()}
                >
                  ${this.installedCities.map(
      (C) => d`<mwc-list-item .value=${C}
                        >${C}</mwc-list-item
                      >`
    )}
                </ha-select>
              </ha-formfield>
            ` : A.integration === "peu" ? d`
                <ha-formfield label="${this._t("location")}">
                  <ha-select
                    .value=${A.location || ""}
                    @selected=${(C) => this._updateConfig("location", C.target.value)}
                    @closed=${(C) => C.stopPropagation()}
                  >
                    ${this.installedPeuLocations.map(
      ([C, D]) => d`<mwc-list-item .value=${C}
                          >${D}</mwc-list-item
                        >`
    )}
                  </ha-select>
                </ha-formfield>
              ` : d`
                <ha-formfield label="${this._t("region_id")}">
                  <ha-select
                    .value=${A.region_id || ""}
                    @selected=${(C) => this._updateConfig("region_id", C.target.value)}
                    @closed=${(C) => C.stopPropagation()}
                  >
                    ${this.installedRegionIds.map(
      (C) => d`
                        <mwc-list-item .value=${C}>
                          ${C} — ${Cg[C] || C}
                        </mwc-list-item>
                      `
    )}
                  </ha-select>
                </ha-formfield>
              `}
        <!-- Titel -->
        <ha-formfield label="${this._t("title")}">
          <ha-textfield
            .value=${A.title || ""}
            placeholder="${this._t("title")}"
            @input=${(C) => {
      const D = C.target.value;
      this._updateConfig("title", D === "" ? void 0 : D);
    }}
          ></ha-textfield>
        </ha-formfield>

        <!-- Layout-switchar -->
        <ha-formfield label="${this._t("minimal")}">
          <ha-switch
            .checked=${A.minimal}
            @change=${(C) => this._updateConfig("minimal", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("allergens_abbreviated")}">
          <ha-switch
            .checked=${A.allergens_abbreviated}
            @change=${(C) => this._updateConfig("allergens_abbreviated", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <!-- Nya switchar för text och värde -->
        <ha-formfield label="${this._t("show_text_allergen")}">
          <ha-switch
            .checked=${A.show_text_allergen}
            @change=${(C) => this._updateConfig("show_text_allergen", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("show_value_text")}">
          <ha-switch
            .checked=${A.show_value_text}
            @change=${(C) => this._updateConfig("show_value_text", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("show_value_numeric")}">
          <ha-switch
            .checked=${A.show_value_numeric}
            @change=${(C) => this._updateConfig("show_value_numeric", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("show_value_numeric_in_circle")}">
          <ha-switch
            .checked=${A.show_value_numeric_in_circle}
            @change=${(C) => this._updateConfig(
      "show_value_numeric_in_circle",
      C.target.checked
    )}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("show_empty_days")}">
          <ha-switch
            .checked=${A.show_empty_days}
            @change=${(C) => this._updateConfig("show_empty_days", C.target.checked)}
          ></ha-switch>
        </ha-formfield>

        <!-- Dag-inställningar -->
        <ha-formfield label="${this._t("days_relative")}">
          <ha-switch
            .checked=${A.days_relative}
            @change=${(C) => this._updateConfig("days_relative", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("days_abbreviated")}">
          <ha-switch
            .checked=${A.days_abbreviated}
            @change=${(C) => this._updateConfig("days_abbreviated", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("days_uppercase")}">
          <ha-switch
            .checked=${A.days_uppercase}
            @change=${(C) => this._updateConfig("days_uppercase", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
        <ha-formfield label="${this._t("days_boldfaced")}">
          <ha-switch
            .checked=${A.days_boldfaced}
            @change=${(C) => this._updateConfig("days_boldfaced", C.target.checked)}
          ></ha-switch>
        </ha-formfield>

        <!-- Antal dagar -->
        <div class="slider-row">
          <div class="slider-text">${this._t("days_to_show")}</div>
          <div class="slider-value">${A.days_to_show}</div>
          <ha-slider
            min="0"
            max="6"
            step="1"
            .value=${A.days_to_show}
            @input=${(C) => this._updateConfig("days_to_show", Number(C.target.value))}
          ></ha-slider>
        </div>

        <!-- Tröskel -->
        <div class="slider-row">
          <div class="slider-text">${this._t("pollen_threshold")}</div>
          <div class="slider-value">${A.pollen_threshold}</div>
          <ha-slider
            min="${n.min}"
            max="${n.max}"
            step="${n.step}"
            .value=${A.pollen_threshold}
            @input=${(C) => this._updateConfig("pollen_threshold", Number(C.target.value))}
          ></ha-slider>
        </div>

        <!-- Sortering -->
        <ha-formfield label="${this._t("sort")}">
          <ha-select
            .value=${A.sort}
            @selected=${(C) => this._updateConfig("sort", C.target.value)}
            @closed=${(C) => C.stopPropagation()}
          >
            ${[
      "value_ascending",
      "value_descending",
      "name_ascending",
      "name_descending"
    ].map(
      (C) => d`<mwc-list-item .value=${C}
                  >${C.replace("_", " ")}</mwc-list-item
                >`
    )}
          </ha-select>
        </ha-formfield>

        <!-- Språk-inställning -->
        <ha-formfield label="${this._t("locale")}">
          <ha-textfield
            .value=${A.date_locale}
            @input=${(C) => this._updateConfig("date_locale", C.target.value)}
          ></ha-textfield>
        </ha-formfield>

        <!-- Allergener (detaljerad) -->
        <details>
          <summary>${this._t("allergens")}</summary>
          <div class="allergens-group">
            ${g.map(
      (C) => d`
                <ha-formfield .label=${C}>
                  <ha-checkbox
                    .checked=${A.allergens.includes(C)}
                    @change=${(D) => this._onAllergenToggle(C, D.target.checked)}
                  ></ha-checkbox>
                </ha-formfield>
              `
    )}
          </div>
        </details>
        <!-- Fraser -->
        <h3>${this._t("phrases")}</h3>
        <div class="preset-buttons">
          <ha-formfield label="${this._t("phrases_translate_all")}">
            <ha-select
              .value=${this._selectedPhraseLang}
              @selected=${(C) => this._selectedPhraseLang = C.target.value}
              @closed=${(C) => C.stopPropagation()}
            >
              ${mC.map(
      (C) => d`
                  <mwc-list-item .value=${C}>
                    ${new Intl.DisplayNames([this._lang], {
        type: "language"
      }).of(C) || C}
                  </mwc-list-item>
                `
    )}
            </ha-select>
          </ha-formfield>
          <mwc-button
            @click=${() => this._resetPhrases(this._selectedPhraseLang)}
          >
            ${this._t("phrases_apply")}
          </mwc-button>
        </div>
        <details>
          <summary>${this._t("phrases_full")}</summary>
          ${g.map(
      (C) => d`
              <ha-formfield .label=${C}>
                <ha-textfield
                  .value=${A.phrases.full[C] || ""}
                  @input=${(D) => {
        const t = {
          ...A.phrases,
          full: { ...A.phrases.full, [C]: D.target.value }
        };
        this._updateConfig("phrases", t);
      }}
                ></ha-textfield>
              </ha-formfield>
            `
    )}
        </details>
        <details>
          <summary>${this._t("phrases_short")}</summary>
          ${g.map(
      (C) => d`
              <ha-formfield .label=${C}>
                <ha-textfield
                  .value=${A.phrases.short[C] || ""}
                  @input=${(D) => {
        const t = {
          ...A.phrases,
          short: { ...A.phrases.short, [C]: D.target.value }
        };
        this._updateConfig("phrases", t);
      }}
                ></ha-textfield>
              </ha-formfield>
            `
    )}
        </details>
        <details>
          <summary>${this._t("phrases_levels")}</summary>
          ${Array.from({ length: I }, (C, D) => D).map(
      (C) => d`
              <ha-formfield .label=${C}>
                <ha-textfield
                  .value=${A.phrases.levels[C] || ""}
                  @input=${(D) => {
        const t = [...A.phrases.levels];
        t[C] = D.target.value;
        const h = { ...A.phrases, levels: t };
        this._updateConfig("phrases", h);
      }}
                ></ha-textfield>
              </ha-formfield>
            `
    )}
        </details>
        <details>
          <summary>${this._t("phrases_days")}</summary>
          ${[0, 1, 2].map(
      (C) => d`
              <ha-formfield .label=${C}>
                <ha-textfield
                  .value=${A.phrases.days[C] || ""}
                  @input=${(D) => {
        const t = { ...A.phrases.days, [C]: D.target.value };
        this._updateConfig("phrases", { ...A.phrases, days: t });
      }}
                ></ha-textfield>
              </ha-formfield>
            `
    )}
        </details>
        <ha-formfield label="${this._t("no_information")}">
          <ha-textfield
            .value=${A.phrases.no_information || ""}
            @input=${(C) => this._updateConfig("phrases", {
      ...A.phrases,
      no_information: C.target.value
    })}
          ></ha-textfield>
        </ha-formfield>

        <!-- Tap Action -->
        <h3>Tap Action</h3>
        <ha-formfield label="Enable tap action">
          <ha-switch
            .checked=${this._tapType !== "none"}
            @change=${(C) => {
      C.target.checked ? this._tapType = "more-info" : (this._tapType = "none", this._updateConfig("tap_action", void 0)), this.requestUpdate();
    }}
          ></ha-switch>
        </ha-formfield>
        ${this._tapType !== "none" ? d`
              <div style="margin-top: 10px;">
                <label>Action type</label>
                <ha-select
                  .value=${this._tapType}
                  @selected=${(C) => {
      this._tapType = C.target.value;
      let D = { type: this._tapType };
      if (this._tapType === "more-info" && (D.entity = this._tapEntity), this._tapType === "navigate" && (D.navigation_path = this._tapNavigation), this._tapType === "call-service") {
        D.service = this._tapService;
        try {
          D.service_data = JSON.parse(
            this._tapServiceData || "{}"
          );
        } catch {
          D.service_data = {};
        }
      }
      this._updateConfig("tap_action", D), this.requestUpdate();
    }}
                  @closed=${(C) => C.stopPropagation()}
                >
                  <mwc-list-item value="more-info">More Info</mwc-list-item>
                  <mwc-list-item value="navigate">Navigate</mwc-list-item>
                  <mwc-list-item value="call-service"
                    >Call Service</mwc-list-item
                  >
                  <mwc-list-item value="none">None</mwc-list-item>
                </ha-select>
              </div>
              ${this._tapType === "more-info" ? d`
                    <ha-formfield label="Entity">
                      <ha-textfield
                        .value=${this._tapEntity}
                        @input=${(C) => {
      this._tapEntity = C.target.value, this._updateConfig("tap_action", {
        type: "more-info",
        entity: this._tapEntity
      });
    }}
                      ></ha-textfield>
                    </ha-formfield>
                  ` : ""}
              ${this._tapType === "navigate" ? d`
                    <ha-formfield label="Navigation path">
                      <ha-textfield
                        .value=${this._tapNavigation}
                        @input=${(C) => {
      this._tapNavigation = C.target.value, this._updateConfig("tap_action", {
        type: "navigate",
        navigation_path: this._tapNavigation
      });
    }}
                      ></ha-textfield>
                    </ha-formfield>
                  ` : ""}
              ${this._tapType === "call-service" ? d`
                    <ha-formfield label="Service (e.g. light.turn_on)">
                      <ha-textfield
                        .value=${this._tapService}
                        @input=${(C) => {
      this._tapService = C.target.value;
      let D = {};
      try {
        D = JSON.parse(this._tapServiceData || "{}");
      } catch {
      }
      this._updateConfig("tap_action", {
        type: "call-service",
        service: this._tapService,
        service_data: D
      });
    }}
                      ></ha-textfield>
                    </ha-formfield>
                    <ha-formfield label="Service data (JSON)">
                      <ha-textfield
                        .value=${this._tapServiceData}
                        @input=${(C) => {
      this._tapServiceData = C.target.value;
      let D = {};
      try {
        D = JSON.parse(this._tapServiceData || "{}");
      } catch {
      }
      this._updateConfig("tap_action", {
        type: "call-service",
        service: this._tapService,
        service_data: D
      });
    }}
                      ></ha-textfield>
                    </ha-formfield>
                  ` : ""}
            ` : ""}

        <!-- Debug -->
        <ha-formfield label="${this._t("debug")}">
          <ha-switch
            .checked=${A.debug}
            @change=${(C) => this._updateConfig("debug", C.target.checked)}
          ></ha-switch>
        </ha-formfield>
      </div>
    `;
  }
  static get styles() {
    return NA`
      .card-config {
        display: flex;
        flex-direction: column;
        gap: 12px;
        padding: 16px;
      }
      ha-formfield,
      details {
        margin-bottom: 8px;
      }
      .allergens-group {
        display: flex;
        flex-wrap: wrap;
        gap: 8px;
      }
      details summary {
        cursor: pointer;
        font-weight: bold;
        margin: 8px 0;
      }
      ha-slider {
        width: 100%;
      }
      ha-select {
        width: 100%;
        --mdc-theme-primary: var(--primary-color);
      }
      .preset-buttons {
        display: flex;
        flex-wrap: wrap;
        gap: 8px;
        margin-bottom: 16px;
      }
      .slider-row {
        display: grid;
        grid-template-columns: auto 3ch 1fr;
        align-items: center;
        gap: 8px;
        margin-bottom: 8px;
      }
      .slider-text {
        /* etikett, naturlig bredd */
      }
      .slider-value {
        /* värdet får alltid 3 teckenplats (t.ex. "0,5" / "1  ") */
        font-family: monospace;
        text-align: right;
        width: 3ch;
      }
      .slider-row ha-slider {
        width: 100%;
      }
    `;
  }
}
customElements.define("pollenprognos-card-editor", OC);
window.customCards = window.customCards || [];
window.customCards.push({
  type: "pollenprognos-card",
  name: "Pollenprognos Card",
  preview: !0,
  description: "Visar en grafisk prognos för pollenhalter",
  documentationURL: "https://github.com/krissen/pollenprognos-card"
});
