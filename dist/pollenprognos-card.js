 ...
 */
var xA;
const pA = window, aA = pA.trustedTypes, zA = aA ? aA.createPolicy("lit-html", { createHTML: (t) => t }) : void 0, HA = "$lit$", _ = `lit$${(Math.random() + "").slice(9)}$`, rg = "?" + _, ug = `<${rg}>`, IA = document, DA = () => IA.createComment(""), dA = (t) => t === null || typeof t != "object" && typeof t != "function", og = Array.isArray, fg = (t) => og(t) || typeof (t == null ? void 0 : t[Symbol.iterator]) == "function", wA = `[ 	
\f\r]`, sA = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, VA = /-->/g, NA = />/g, gA = RegExp(`>|${wA}(?:([^\\s"'>=/]+)(${wA}*=${wA}*(?:[^ 	
...
async function qn(t, A) {
  const g = !!A.debug, e = (c) => c.charAt(0).toUpperCase() + c.slice(1), n = q(t, A.date_locale), C = A.date_locale || z.date_locale, a = A.days_relative !== !1, I = !!A.days_abbreviated, i = !!A.days_uppercase, o = A.phrases || {}, D = o.full || {}, h = o.short || {}, r = o.levels, l = Array.isArray(r) && r.length === 7 ? r : Array.from({ length: 7 }, (c, s) => O(`card.levels.${s}`, n)), E = o.no_information || O("card.no_information", n), x = o.days || {}, b = A.days_to_show ?? z.days_to_show, B = A.pollen_threshold ?? z.pollen_threshold;
  ...
  for (const c of A.allergens)
    try {
      ...
      const m = t.states[S], w = j(m.state), v = j(m.attributes[Tn]), f = j(m.attributes[zn]), J = [
        { date: Y, level: w },
